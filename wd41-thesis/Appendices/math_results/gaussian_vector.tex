...
That is, the random variables have a \gls[hyper=false]{mvn} distribution. 
...
From an epistemological viewpoint, a variable modeled as \gls[hyper=false]{mvn} distribution is a particular way of quantifying uncertainty about that variable.
...
The marginal density of both $\bm{\mathcal{Z}}_A$ and $\bm{\mathcal{Z}}_B$ follows an \gls[hyper=false]{mvn} distribution given by,
\marginpar{Gaussian identity: marginal density}
\begin{equation}
		p(\mathbf{z}_A) = \frac{1}{(2\pi)^{D_1/2}|\boldsymbol{\Sigma_{A,A}}|^{1/2}} \exp{\left[-\frac{1}{2}(\mathbf{z}_A-\boldsymbol{\mu}_A)^T\boldsymbol{\Sigma^{-1}_{A,A}}(\mathbf{z}_A-\boldsymbol{\mu}_B)\right]}
...
\begin{equation}
	\begin{split}
		& p(\mathbf{z}_A|\mathbf{z}_B) = \frac{1}{2\pi^{D_1/2}|\boldsymbol{\Sigma_{A,A}}|^{1/2}} \exp{\left[-\frac{1}{2}(\mathbf{z}_A-\boldsymbol{\mu}_A)^T\boldsymbol{\Sigma^{-1}_{A,A}}(\mathbf{z}_A-\boldsymbol{\mu}_B)\right]} \\
