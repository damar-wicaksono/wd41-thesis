...
This section of the appendix provides some of the most necessary concepts of Markov chain stochastic process,
especially in relation to its application for Monte Carlo simulation (i.e., \gls[hyper=false]{mcmc}).
The associated theorems are presented without proof though the list of references are provided.
Many of the concepts are first introduced, and some are exclusively, for the discrete-state Markov chain.
It provides a more intuitive entry to the theory of Markov chain through matrix notation and a graphical representation.
A generalization to the continuous-state Markov chain will then be attempted,
though many important equivalent theorems are difficult to even state let alone proof without the prerequisite notions involving measure theory.
In such cases, references will be provided. 
...
% Markov Chain
A Markov chain on a discrete-state space $\mathcal{S}$ is defined as a sequence of random variables $\{\mathcal{X}^{(i)}; i \geq 0\}$ where the indices represents successive time, step, or iteration,
...
\begin{itemize}
	...
	\item The \emph{state space} $\mathcal{S}$, all the possible outcomes of the random variables $\{\mathcal{X}^{(i)}\}$.
	...
	\begin{equation}
		\mathbb{P}(\mathcal{X}^{(0)} = x) = \pi^{(0)}_x \,\,\, \forall x \in  \mathcal{S}
...
the engineering is done somewhat in reverse (``Given a density/distribution, construct $P$'').
...
A Markov chain with transition probability matrix $P$ satisfies the detailed balance condition if there exists a probability distribution $\pi$ such that,
\begin{equation}
...
For proof of this theorem see \cite{Kelly1978,Robert2004}.
