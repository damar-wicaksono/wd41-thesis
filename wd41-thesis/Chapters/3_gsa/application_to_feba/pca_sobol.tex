...
The estimated Sobol' indices for the first \gls[hyper=false]{fpc} of the registered clad temperature transient at the mid-height of the assembly are given in Fig.~\ref{fig:ch3_plot_sobol_boxplot_tc4_pc1}.
...
The other parameters are proved to be almost noninfluential with high certainty in the estimates.
...
\bigfigure[pos=tbhp,
           ...
           shortcaption={Sobol Indices estimates with the time of quenching as \gls[hyper=false]{qoi}}]
{../figures/chapter3/figures/plotSobolBoxplotTC4RegPC1}
{The sensitivity indices with respect to the $1$\textsuperscript{st} \gls[hyper=false]{fpc} of the (registered) clad temperature transient at the mid-height of the assembly. The boxplots represent the bootstrap samples quartile statistics, while the vertical line of each box encompasses the $95\%$ percentile of the sample.}

% 2nd fPC of Registered TC4
Fig.~\ref{fig:ch3_plot_sobol_boxplot_tc4_pc2} shows the Sobol' sensitivity indices using the scores associated with the second \gls[hyper=false]{fpc} as the \gls[hyper=false]{qoi}.
...
{The sensitivity indices with respect to the $2$\textsuperscript{nd} \gls[hyper=false]{fpc} of the (registered) clad temperature transient at the mid-height of the assembly. The boxplots represent the bootstrap samples quartile statistics, while the vertical line of each box encompasses the $95\%$ percentile of the sample.}
...
{The sensitivity indices with respect to the $1$\textsuperscript{st} \gls[hyper=false]{fpc} of the warping function for the clad temperature transient at the mid-height of the assembly. The boxplots represent the bootstrap samples quartile statistics, while the vertical line of each box encompasses the $95\%$ percentile of the sample.}
...
{The sensitivity indices with respect to the $1$\textsuperscript{st} \gls[hyper=false]{fpc} of the pressure drop transient at the middle of the assembly. The boxplots represent the bootstrap samples quartile statistics, while the vertical line of each box encompasses the $95\%$ percentile of the sample.}
...
{The main-effect and total-effect sensitivity indices with the maximum clad temperature as the \gls[hyper=false]{qoi}. The boxplots represent the bootstrap samples quartile statistics, while the vertical line of each box encompasses the $95\%$ percentile of the sample.}

% Closing Paragraph
The numerical results of the estimated Sobol' indices presented above are tabulated in Table~\ref{tab:app_sobol_tc4_pc1} through \ref{tab:app_sobol_co_pc1} in Appendix~\ref{app:tbl_results_sobol}.
To give a measure of uncertainties on the estimates due to \gls[hyper=false]{mc} sampling, 
the results in the table are complemented by the $95\%$ bootstrap percentile confidence interval $CI_{pct}$, which was based on the percentile method \cite{Efron1986}.