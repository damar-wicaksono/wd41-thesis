...
{The main-effect and total-effect sensitivity indices with the maximum mid-height clad temperature as the \gls[hyper=false]{qoi}. Each boxplot represents the bootstrap sample quartile statistics and the vertical line extends the $95$th sample percentile.}
...
the mid-height clad temperature at each time step was taken as the \gls[hyper=false]{qoi} and the main-effect indices were calculated.
...
{(top) the main-effect sensitivity indices at different time steps during the reflood transient. (bottom) The clad temperature standard deviation at different time steps during the same transient.}
...
After quenching, the clad surface temperature is essentially commensurate with the coolant temperature.
