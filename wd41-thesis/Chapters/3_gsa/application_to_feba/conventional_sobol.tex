...
As shown in Fig.~\ref{fig:ch3_plot_sobol_boxplot_tc4_maxtemp} the variation of the maximum clad temperature at the middle of the assembly (with standard devitaion of $59.7\,[K]$ was driven mainly by four model parameters,
contributing to $77\%$ of the total output variation.
One influential parameter is related to the spacer grid HT enhancement model and the three others are related to the \gls[hyper=false]{dffb} regime model parameters (with the combined effect contributing to $63\%$ of the total output variation).
Moreover since the sum of all the main-effect indices was relative close to $1.0$, the parameters were not interacting with respect to this particular \gls[hyper=false]{qoi}.
\bigfigure[pos=tbhp,
					 opt={width=0.925\textwidth},
           label={fig:ch3_plot_sobol_boxplot_tc4_maxtemp},
           shortcaption={Sobol Indices estimates with the maximum clad temperature as \gls[hyper=false]{qoi}.}]
...
The numerical results of the Sobol' indices estimates for both main-effect and total-effect for each parameter with respect to these two outputs are tabulated in the Appendix~\ref{app:tbl_results_sobol}.
