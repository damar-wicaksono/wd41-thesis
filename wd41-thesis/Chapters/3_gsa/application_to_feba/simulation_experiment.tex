...
All the steps required in the application \gls[hyper=false]{fda} were already demonstrated in the context of the reflood simulation output in \cite{Wicaksono2014a}.
...
The application of \gls[hyper=false]{fda} resulted in a set of common \gls[hyper=false]{fpc} and a set of realization specific \gls[hyper=false]{fpc} score.
The score was therefore used as the \gls[hyper=false]{qoi} for the Sobol' indices estimation
...
the $95$\% percentile \gls[hyper=false]{ci} were constructed using the bootstrap technique (see \cite{Efron1986} and Appendix.~\ref{app:bootstrapping} for detail).
