...
the outer loop samples values of $X_d$ while the inner loop samples values of $\bm{\mathcal{X}}_{\sim d}$ (all parameters except $x_d$).
...
In the inner loop, the arithmetic mean of the model output is taken for a given value of $X_d$ but over many values of $\bm{\mathcal{X}}_{\sim d}$. 
