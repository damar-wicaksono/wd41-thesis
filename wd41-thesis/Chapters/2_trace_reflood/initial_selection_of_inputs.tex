...
Different codes share similarity in terms of the models representing distinct flow regimes (high-level) but might differ in the constituents of each of these models (lower-level).
By focusing on the higher-level of the models implementations allows, to some extent, to use reference uncertainty information obtained from codes other than \gls[hyper=false]{trace}.
