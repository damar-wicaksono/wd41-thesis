...
% Opening paragraph
The quantified prior uncertainties of the input parameters are propagated through the \gls[hyper=false]{trace} model of \gls[hyper=false]{feba} to assess (and verify) the prior level of prediction uncertainties before experimental data is used to update the prior uncertainties of the model parameters.
The prior uncertainty propagation step consists generating independent samples of values for each of the $27$ selected input parameters from their prior \glspl[hyper=false]{pdf} (Tables~\ref{tab:trace_model_parameter_1}-\ref{tab:trace_model_parameter_2}).
...
Note that in Fig.~\ref{fig:ch2_plot_base_case_216_dp} the transient between the two equilibrium values indicates the flooding of the channel between axial level $1.7$ and $2.3\,[m]$ from initially pure steam (low pressure drop) to mixture (rising) and to eventually pure liquid (higher pressure drop). 
...
As observed, there is no major discrepancy between the \gls[hyper=false]{trace} predictions and the experimental data and the prediction uncertainty bands cover the experimental data especially during the transient (i.e., the ramp between two equilibrium values) well.
\bigfigure[pos=!h,
