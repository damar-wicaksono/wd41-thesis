...
\emph{Reflood} phase is the last phase of the $4$ canonical phases in the mitigation of \gls[hyper=false]{lbloca} in an \gls[hyper=false]{lwr}; the other $3$ preceding phases are the \emph{blowdown} phase, \emph{refill} phase, and \emph{bypass} phase (see \cite{Hewitt2000} for an introduction on the transient).
\marginpar{Reflood phase in LBLOCA}
It refers to the phase of the transient in which emergency coolant water flows slowly upward through the reactor core, quenching the fuel elements along the way. 
...
% Reflooding
\emph{Quenching} (or \emph{rewetting}) refers to the phenomenon in which a sustainable contact between the liquid phase of the coolant and the hot surface of the fuel elements is re-established.
\marginpar{Quenching}
...
Keeping the power constant increases the cladding temperature up until more mixture of steam and liquid (droplets) arrives at the location, improving the \gls[hyper=false]{ht} mechanism, and turn ingthe temperature around from the maximum ($T_\text{max.}$).
...
Moreover as the quenching happens below this particular location, large temperature gradient in the axial location is present and is accelerating the heat conduction from the un-quenched part to the quenched part of the cladding.
...
Phenomenological view of the process, as adopted by \gls[hyper=false]{trace} code \cite{USNRC2012}, is shown in Fig.~\ref{fig:ch2_reflood_phenomenology} along with the corresponding part in the reflood curve of Fig.~\ref{fig:ch2_reflood_curve}.
\marginpar{Reflood, phenomenology}
The post-\gls[hyper=false]{chf} flow regimes (regimes $(2)$--$(5)$ in the figure) are in-between two pre-\gls[hyper=false]{chf} flow regimes, namely nucleate boiling at the bottom and and steam convection at the top.
...
Far from the quench from, liquid droplets are dispersed and carried away by the bulk steam flow.
...
The droplets also provide heat sink from the bulk steam flow.
The presence of the droplets also further enhance the turbulence of the steam flow improving the convection from the wall to the steam flow \cite{Zeng2010}.
The improvement to \gls[hyper=false]{ht} brought by these mechanism allows the cladding temperature to reach maximum and turn.

% Inverted Slug
As the quench front progresses upward, in the intermediate distance from the front, more efficient cooling is provided from morphologically less regular entrained liquid, called ligaments or slugs ($3$ in Fig.~\ref{fig:ch2_post_chf_regime}).
\marginpar{Inverted Slug}
Due to this efficient cooling, the cladding temperature keeps decreasing.
The slug flow regime is inverted in the sense that the at the center of the flow liquid phase is the predominant phase. 
In \gls[hyper=false]{trace} these slugs are modeled as prolate ellipsoids.
The flow regime itself represents an interpolatory region between the previous \gls[hyper=false]{dffb} flow regime and the subsequent \gls[hyper=false]{iafb} flow regime \cite{USNRC2012}.

% IAFB
In the short distance from the front, the bulk of the subcooled liquid flow starts to appear in front of the surface.
\marginpar{\Glsfirst[hyper=false]{iafb}}
However, a thin vapor film still separates the liquid from the wall and thus prevent ideal \gls[hyper=false]{ht} to occur.
...
It results in a very large \gls[hyper=false]{htc} between the wall to the fluid and causes the rapid drop (i.e., quenching) of the temperature ($5$ in Fig.~\ref{fig:ch2_post_chf_regime}). 
After quenching and the cladding surface is in full contact with the liquid, the cladding temperature is in equilibrium with bulk flow of saturated liquid and the flow regime involves belongs to different phenomenology ($6$ in Fig.~\ref{fig:ch2_post_chf_regime}) (in \gls[hyper=false]{trace}, it belongs to the pre-\gls[hyper=false]{chf} package \cite{USNRC2012}).