...
\begin{table}[ht]
	...
	\begin{tabularx}{\textwidth}{Xcc} 
		...
		\midrule
		mass rate of change  					 & $\frac{\partial [(1-\alpha)\rho_l]}{\partial t}$  & $\frac{\partial [\alpha \rho_g]}{\partial t}$ \\
		mass convection rate 					 & $\nabla \cdot [(1-\alpha) \rho_l \mathbf{v_l}]$   & $\nabla \cdot [\alpha \rho_g \mathbf{v_g}]$ \\
		interfacial mass-transfer rate & $- \Gamma$                                        & $\Gamma$ \\
		\bottomrule
	\end{tabularx}
\end{table}
...
\begin{table}[ht]
	...
	\begin{tabularx}{\textwidth}{>{\raggedright}Xcc} 
		...
		\tableheadline{Terms} & \tableheadline{Liquid Phase} & \tableheadline{Gas Phase} \\ 
		\midrule
		momentum rate of change  					 	& $\frac{\partial [(1-\alpha)\rho_l \mathbf{v}_l]}{\partial t}$  				& $\frac{\partial [\alpha \rho_g \mathbf{v}_g]}{\partial t}$ \\
		...
		pressure gradient                  	& $(1 - \alpha) \nabla p$                                        				& $\alpha \nabla p$ \\
		\midrule
		momentum change due to:           	&                                                                 			& \\
		interfacial friction 								& $\mathbf{f}_i$ 																												&  $- \mathbf{f}_i$ \\
		wall friction 											& $\mathbf{f}_{wl}$ 																										& $\mathbf{f}_{wg}$ \\
		body force 													& $(1 - \alpha) \rho_l \mathbf{g}$ 																			& $\alpha \rho_g \mathbf{g}$ \\
		interfacial mass-transfer 					& $- \Gamma \mathbf{v}_i$ 																							& $\Gamma \mathbf{v}_i$ \\
		\bottomrule
	\end{tabularx}
\end{table}
...
\gls{pg} is the partial pressure of the gas phase;
