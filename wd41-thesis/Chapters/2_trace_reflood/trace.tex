...
As such, system thermal-hydraulics distinguishes itself by considering explicitly the geometry, materials, boundary conditions, various interconnecting components, and control systems that constitute an \gls[hyper=false]{npp} \cite{DAuria2012}.
...
where two-phase flow can be expected to occur during its operation, in normal operating condition for \glspl[hyper=false]{bwr} and accident condition for \glspl[hyper=false]{pwr}.
\begin{figure}[bth]
	\centering
...
This due to the fact that in two-phase flow, the morphological configurations of the flow can vary widely depending on many parameters such as difference in phase density, phasic velocity, and flow orientation.
...
This is illustrated in the Fig. where a hypothetical probe is put within a two-phase flow and a signal of indicator function $M(\mathbf{r}, t)$ is recorded.
%Figure
...
The solution of local instantaneous formulation of two-phase flow requires the motion of interfacial surface to be resolved. 
...
Though advances have been made in the area of Computational Fluid Dynamics in this regard, 
...
Averaging the flow state variables in time and volume results in the so-called \emph{two-fluid model} \cite{Ishii2011}.
...
As the averaging incurs loss of information regarding the flow local information (such as the interfacial structure between the phases), 
