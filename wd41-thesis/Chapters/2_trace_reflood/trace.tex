...
% Major difficulty
The remaining problem of modeling properly the two-phase flow in a heated channel, though much more limited in scope, is still a difficult task.
The due to the fact that in two-phase flow, the morphological configurations of the flow varies widely, affected by many flow parameters and fluid properties.
Different morphological configuration implies different interfacial structure between the two phases (see figure), 
which in turn affects the mass, momentum, and energy coupling terms (\emph{transfer relation}) between the phases.
Put differently,  
The wide range of morphological configurations, characterized by different interfacial structure between the two phases (see figure) in turn affects the   
Though mass, momentum, and energy conservation equations remain a valid mathematical model remains 
The solution of local instantaneous formulation of two-phase flow, where the interface structure fully resolved, is still intractable for the purpose of system thermal-hydraulics analysis.

% Major simplification
 
Most system code, such as trace, adopted a component-based approach to modeling a reactor system.
In such approach the


Indeed, according to D'Auria the distinguishing feature of system code is the ca.
And yet, at its heart, the essence of the problem remains, 
...
Time- and volume-averThere major, but common simplifications adopted in the modeling of nuclear reactor systems by system codes.
 
Though three-dimensional treatment of flow is available in some, the flow in majority of hydraulic components are still treated as one-dimension.
