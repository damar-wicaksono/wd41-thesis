...
% What is system code
Thermal-hydraulics system code is a computer code used to analyze thermal-hydraulics behavior of nuclear power plants \cite{Roth2014}.
...
% What is inside system code and how it is being used
To model properly the thermal-hydraulics behavior of nuclear power plants, many aspects.

% Component-system code

% The essence of the problem tackled in thermal-hydraulics system code
As such, system thermal-hydraulics distinguishes itself by considering explicitly the geometry, materials, boundary conditions, and control systems of an operating nuclear power plant \cite{DAuria2012}.
Yet, it can be argued that the modeling of two-phase flow inside a heated channel remains a central part in nuclear thermal-hydraulics analysis 
which puts an emphasis in the correct prediction of cladding temperature evolution during different postulated scenarios. 
This is also supported by the fact that the majority of operating nuclear power plants is of Light Water Reactor type, 
where two-phase flow can be expected to occur during its operation\footnote{in normal operating condition for BWR, and accident condition for PWR}.
\begin{figure}[bth]
	\centering
	\includegraphics[scale=0.55]{../figures/heatedChannel/heatedChannel.pdf}
	\caption[Illustration of Heated Channel]{System thermal-hydraulics analysis encompasses many aspects of nuclear system thermal-hydraulics, but the core of the problem for predicting the cladding temperature evolution (especially during accident condition) is to model the coolant flow in a heated channel in steady or transient condition. Here it is shown a simplified picture of loss of coolant accident in a \glsentryshort{pwr} where phase change along the heated channel occurs.}
	\label{fig:heated_channel}
\end{figure}

% Major difficulty
This remaining problem of modeling properly the two-phase flow in a heated channel, though much more limited in scope, is far from trivial.
This due to the fact that in two-phase flow, the morphological configurations of the flow can vary widely.
Different morphological configuration implies different interfacial surface structure between the two phases (see figure), 
which in turn affects the mass, momentum, and energy coupling terms (\emph{transfer relation}) between the phases.
Additionally, the interfacial surface and its deformation in an arbitrary flow configuration are not known \emph{a priori} and becomes part of the solution.

% Rigorous approach
Rigorous solution of local instantaneous formulation of two-phase flow respects, on top of the mass, momentum, and energy balance of each phase, the \emph{topological constraint} of the flow.
The constraint states that only one phase can exist at any given time and at any given location in the flow.
This is illustrated in the Fig. where a hypothetical probe is put within a two-phase flow and a signal of indicator function $\alpha(\mathbf{r}, t)$ is recorded.
The indicator function\footnote{the indicator function defined above is equivalent to the local instantaneous void fraction, which can also be interpreted as the probability that the gas phase is present at a given location at a given moment} is defined as,
\begin{equation}
  \alpha (\mathbf{r}, t) =
    \begin{cases}
      1 \, ; \, \text{if} A \in \text{gas} \\
      0 \, ; \, \text{otherwise}
    \end{cases}
\label{eq:indicator_function}
\end{equation}
where 
The solution of local instantaneous formulation of two-phase flow requires the motion of interfacial surface is resolved. 
As the time and length scales of two-phase flow of an arbitrary morphological configurations can vary wildly, 
the problem of resolving the interfacial surface becomes intractable. 
Though advances have been made in the area of Computational Fluid Dynamics (such as PSI boil) in this regards, 
the problem remains intractable for the purpose of thermal-hydraulics system analysis.

% Time and Volume Averaged
...
Averaging can be seen as a filtering operation to remove the temporal and spatial fluctuation (short scale variation) in the flow.
...
The volume over which averaging is carried out is referred to as a \emph{control volume} or a \emph{cell}.
...
\end{equation}
Following the above formulation, void fraction can be interpreted as the fraction of the control volume occupied by the gas phase (cite TRACE). 
...
\begin{figure}[bth]
	...
	\caption[Illustration of Flow Averaging]{}
	\label{fig:flow_averaging}
\end{figure}

Time and volume average the states variables in the conservation equation results in the so-called two-fluid six equation model (cite ISHII).
This is the state of the art formulation for describing two-phase flow in system codes, which include CATHARE, RELAP5, and TRACE.
In the subsequent section the final formulation of the governing equations (time- and volume-averaged) adopted in TRACE are summarized. 
Complete derivation of this formulation can be found in (TRACE theory manual)
