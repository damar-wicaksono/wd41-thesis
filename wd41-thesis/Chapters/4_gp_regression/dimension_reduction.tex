...
An approach proposed in \cite{Kleijnen2000} is to represent the multiple outputs by separate metamodels.
...
$\bm{f}:\bm{X} \subseteq \mathbb{R}^D \mapsto \mathbb{R}^P$ where $P$ is the dimension of the output parameter space.
		...
		\mathbf{Y}(\mathbf{DM}) & = 
			\begin{pmatrix}
				...
				\rule[.5ex]{2.5em}{0.4pt}	& \mathbf{y}_n	&	\rule[.5ex]{2.5em}{0.4pt} \\
																	& \vdots	&
			\end{pmatrix} \\
		\mathbf{y}_n & = (\text{y}_{n,1}, \cdots, \text{y}_{n,p}, \cdots, \text{y}_{n,P}) \\
		             & = (y(\bm{x}_n)_1, \cdots, y(\bm{x}_n)_p, \cdots, y(\bm{x}_n)_P)
	\end{split}
\label{eq:raw_output}
\end{equation}
...
\begin{equation}
	\begin{split}
		\bar{\mathbf{y}} & = [\bar{\text{y}}_1, \cdots, \bar{\text{y}}_p, \cdots, \bar{\text{y}}_P] \\
...
The column vector elements of $\mathbf{V}$ are the principal components of the data set which describe the main directions of the data set, along which the variance of the data set is the largest.
...
That is, only the first $Q$ columns of $\mathbf{V}^*$ are retained with $Q << P$.
...
where a prediction at an arbitrary input $\bm{x}_o \in \mathbf{X}$ is made,
\begin{equation}
