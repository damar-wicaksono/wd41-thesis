...
The construction invol that affected the resulting metamodel's predictive performance.
...
Multiple points per training sample size correspond to \gls[hyper=false]{gp} metamodels constructed using different experimental designs and covariance kernel functions, including multiple replications for each experimental design.
\bigfigure[pos=!tbhp,
...
Furthermore the plot also shows that the variations in the performance become smaller and smaller with increasing sample size.
