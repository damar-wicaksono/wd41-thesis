...
As such, unless the score is perfectly predicted, this error might offset the potential benefit of adding \gls[hyper=false]{pc} for the reconstruction.
...
To put the error into context, for the clad temperature output,
retaining $7$ \glspl[hyper=false]{pc} for the \gls[hyper=false]{gp} \gls[hyper=false]{pc} metamodel gives a reconstruction error of $\approx 22 \, [K]$ (\gls[hyper=false]{rmse}).
This value is small in comparison with the standard deviation of the output in the test data itself, $254 \, [K]$ (less than $9\%$, Table~\ref{tab:ch4_gp_testing}).
The same approach applies to the pressure drop output $10$ \glspl[hyper=false]{pc} gives a reconstruction error of $\approx 78 \, [Pa]$, compared with the test data standard deviation of $9200 \, [Pa]$, or less than $0.9\%$);
and the liquid carryover output ($10$ \glspl[hyper=false]{pc} gives a reconstruction error of $0.27 \, [kg]$, compared with the test data standard deviation of $30.4 \, [kg]$, or less than $0.9\%$).
Note that those numbers are based on a training sample of size $1'920$ and a testing sample of size $5'000$.
As shown in Fig.~\ref{fig:ch4_plot_q2_pcs}, the size of the testing sample is (much) more than enough to obtain stable estimates of these errors.
...
The choice of covariance function has some effect in the predictive performance and its variation across replication. Insofar the choice is between the smoother functions (e.g., the Gaussian, the Mate\'rn $5/2$) and the rougher ones (e.g., the power-exponential, the Mate\'rn $3/2$), 
...
The choice of experimental design, on the other hand, has a marginal effect on the predictive performance across different sample sizes, covariance functions, and replications.
...
but according to Fig.~\ref{fig:ch4_plot_q2_pcs}, that many points might not be necessary.
The size of validation/testing data set can be constrained by first making a plot similar to the one in Fig.~\ref{fig:ch4_plot_q2_pcs} but with an initial small number of samples to first check the convergence of the error estimate before creating unnecessarily large set upfront.