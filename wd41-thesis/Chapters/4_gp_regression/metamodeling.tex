...
\begin{itemize}
	...
	\emph{Universal Kriging} (UK), on the other hand, refers to class of Kriging where a general polynomial model is assumed, but its coefficients are unknown \cite{Koehler1996,Ginsbourger2009,Marrel2008},
	\begin{equation}
		\mu(\mathbf{x}) = \sum_{j=0}^{k} \beta_j h_j(\mathbf{x})
	\label{eq:trend_polynomial}
	\end{equation}
	where $h_j$ is a set of polynomials basis function; and $\beta_j$ is the set of the associated coefficients.
