...
Given a deterministic computer simulator (which again, in essence is a function) $f:\mathbf{x} \in \mathcal{X} \subseteq \mathbb{R}^D \mapsto \mathbb{R}$ 
...
the objective of regression is to compute (or \emph{predict}) the value of $\mathbf{x}_o$ with $\mathbf{x}_o \notin \mathbf{DM}$.
...
As before, the domain $\boldsymbol{X}$ is often rescaled such that $\mathbf{x} \in [0,1]^D$.
...
As such, the response surface of the function has to be reconstructed or estimated based only on the given training data before the prediction is made.
...
The prediction made at particular value of $\mathbf{x}_o$ is made based on the process as estimated according to the training data\footnote{The frequentistic case is the classical approach with a geostatistical analysis method due to Krige dating back to the 1950s \cite{Krige1951} and Matheron to the 1960s \cite{Matheron1963}}.
...
In the frequentistic case, the error is defined as the mean squared of error between the prediction made by the estimated process and (hypothetical) true process;
