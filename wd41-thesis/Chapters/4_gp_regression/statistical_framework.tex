...
evaluated at $\mathbf{DM}$, an experimental design matrix $\{\bm{x}_n\}_{n=1}^N$,
yielding $N$ outputs $\mathbf{y} = \{f(\bm{x}_n) = y_n\}_{n=1}^N$, 
the objective of the regression is to compute (or \emph{predict}) the value of $f(\bm{x}_o)$ with $\bm{x}_o \notin \mathbf{DM}$.

The set $\mathcal{D} \equiv \{(\mathbf{DM}, \mathbf{y})\} = \{(\bm{x}_n, f(\bm{x}_n) = y_n)\}_{n=1}^N$ of $N$ observations is often referred to as the training data,
