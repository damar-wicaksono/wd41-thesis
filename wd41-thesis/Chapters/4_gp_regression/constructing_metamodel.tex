...
In this strategy, a separate validation dataset (samples) is created by generating a new set of validation inputs at which the simulator is evaluated.  
Using random sampling the possibility of having exactly the same training and validation data is excluded.
...
For a computationally expensive simulator, it is not always possible to generate large (if any) independent validation samples.
