...
This also implies that the uncertainties associated with the \gls[hyper=false]{ml} estimates are not incorporated into the Kriging predictor and variance \cite{Helbert2009}.
That is, the uncertainties of the predictor (i.e., its variance) given the observed data is also conditional on a particular set of hyper-parameters values which underestimate the true Kriging variance \cite{Hertog2006}.
