...
% Introductory
In most metamodeling applications, the values of the hyper-parameters for a select \gls[hyper=false]{gp} metamodel are not known a priori.
The parameter estimation process, a term interchangeably used with \emph{fitting}, \emph{training}, and \emph{learning}, 
applies mathematical techniques to a set of training data to estimate the values of hyper-parameters \cite{Kleijnen2000}.
In the following it is assumed that a particular correlation function has been selected and the mean function represented using a set of basis fucntion is assumed to be known.
That is, it starts from the Simple Kriging formulation.
...
it is should be first acknowledged that under \gls[hyper=false]{gp} model, the distribution of the observed data given a Gaussian process $\mathbf{y} \, | \, \mathcal{Y}(\mathbf{x}); \boldsymbol{\Psi}$ is Gaussian such that
\begin{equation}
...
Finally, it is also implied in the formulation that the chosen \gls[hyper=false]{gp} is fully specified through its hyper-parameterization $\boldsymbol{\Psi}$ on $\sigma^2$ and $\mathbf{R}$
...
\begin{equation}
	\hat{\sigma}^2; \boldsymbol{\Theta} = \underset{\boldsymbol{\beta},\sigma^2}{\arg\min} - \ln \mathcal{L} (\hat{\sigma}^2 ; \hat{\boldsymbol{\Theta}})
...
The estimated $\hat{\sigma}^2$ are then fed back to Eq.~(\ref{eq:gp_likelihood}) to obtain the so-called \emph{concentrated/profile likelihood} \cite{Cole2013,Kreutz2013}.
...
\begin{equation}
	\hat{\boldsymbol{\Theta}} ; \hat{\sigma}^2 = \underset{\boldsymbol{\Theta}}{\arg\min} - \ln \mathcal{L} (\hat{\boldsymbol{\Theta}};\hat{\sigma}^2)
	\label{eq:theta_ml}
\end{equation}
...
\begin{equation}
	\hat{m}_{SK}(\mathbf{x}_o) = \mathbf{f}_o \boldsymbol{\beta} + \mathbf{r}^T_{o,\hat{\boldsymbol{\Theta}}} \mathbf{R}^{-1}_{\hat{\boldsymbol{\Theta}}} (\mathbf{y} - \mathbf{f}_o \boldsymbol{\beta})
...
The Eqs.~(\ref{eq:sk_predictor_ml}-\ref{eq:sk_variance_ml}) are the same as Eqs.~(\ref{eq:mean_sk}-\ref{eq:variance_sk}), 
except now the hyper-parameters are replaced by their \gls[hyper=false]{ml} estimates.
\marginpar{Uncertainty on $\boldsymbol{\Psi}$, full Bayesian treatment}
They also imply that the uncertainties associated with the \gls[hyper=false]{ml} estimates are not incorporated into the Kriging predictor and variance \cite{Helbert2009}.
In other words, the uncertainties of the predictor (i.e., its variance) given the observed data is also conditional on a particular set of hyper-parameters values which is the underestimator of true Kriging variance \cite{Hertog2006}.
...
It assumes a prior over the hyper-parameters, compute the posterior, and then use the posterior to average (integrate) the hyper-parameters out from the Kriging predictor and variance \cite{Kennedy2001,Bayarri2007,Higdon2008,Helbert2009}.
This increases the computational cost as well as the complexity of the analysis with mixed results \cite{Chen2006}.
As noted in Bayarri, whose ultimate goal was model calibration against experimental data, the answers provided by either analysis (\gls[hyper=false]{ml} estimates and full Bayesian) are equivalent as the effect of model parameters uncertainties tend to dominate the effect of hyper-parameters uncertainties.
...
is subjected to validation process presented in the next section.
...
As such, its validation is a necessary step in applying the metamodel, at any given input, as a surrogate of the original computer simulator with confidence.
...
The gold standard of validation strategy is by an independent validation (holdout) data \cite{Chen2016}.
...
The cross-validation is an alternative approach to validate a metamodel in this situation \cite{Meckesheimer2002,Iooss2010}.
In cross-validation, a batch of samples (\emph{subsamples}) are removed from the available training samples, 
...
The procedure is repeated by selecting randomly the elements of the removed batch.
...
However, it can potentially be expensive if numerous metamodel fitting is to be carried out (such as in the case of the LOO approach).
...
The accuracy of the metamodel for a particular inputs, however, can still be poor and in the case of \gls[hyper=false]{gp} metamodel is defined probabilistically.  

% Predictivity Coefficient
A particular validation metric that quantifies such global accuracy is the predictivity coefficient $Q_2$ \cite{Iooss2010}.
...
\begin{equation}
	Q_2 (\hat{\mathbf{y}}^*, \mathbf{y}^*) = 1 - \frac{\sum_{n=1}^{N_{valid}} (y^*_n - \hat{y}^*_n)^2}{\sum_{n=1}^{N_{valid}} (\bar{y}^* - y^*_n)^2}
\label{eq:predictivity_coefficient}
\end{equation} 
where $\mathbf{y}^*$ is the outputs at validation inputs produced by the simulator;
$\bar{\mathbf{y}}^*$ is the predictions at validation inputs made by the metamodel;
