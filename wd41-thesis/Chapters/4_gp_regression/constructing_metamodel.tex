...
The selection of points in the input parameter space, which determines the geometrical configuration of $\mathbf{DM}$, is aimed at exploring the whole input parameter space $\mathcal{X}$, at least in the region where the important features of the model (e.g., region of strong non-linearity) are located.
...
Finally, it is also implied in the formulation that the chosen \gls[hyper=false]{gp} is fully specified, through its hyper-parameterization $\boldsymbol{\Psi}$, by $\sigma^2$ and $\mathbf{R}$
such that the notation $\mathcal{Y}(\mathbf{x})$ is removed from the expression.
...
In other words, the uncertainties of the predictor (i.e., its variance) given the observed data is also conditional on a particular set of hyper-parameters values which underestimate the true Kriging variance \cite{Hertog2006}.
...
As noted in Bayarri, whose ultimate goal was model calibration against experimental data, 
