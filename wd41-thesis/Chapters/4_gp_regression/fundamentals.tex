...
To establish the connection with the notion of \emph{random function}, the collection of the above random variables refers to the collection of values of a random function $Y(\circ)$ at various possible input $\mathbf{x}$ in the domain $\chi$.
Specifically, $Y(\mathbf{x}), \, \text{for} \, \mathbf{x} \in \chi \subseteq \mathbb{R}^D$ is a \emph{Gaussian process} if and only if for any choice from the finite set of $\{\mathbf{x}_1, \mathbf{x}_2, \cdots, \mathbf{x}_L ; \, L \geq 1\}$, the vector $\left[Y(\mathbf{x}_1), Y(\mathbf{x}_2), \cdots, Y(\mathbf{x}_L)\right]$ is a multivariate Gaussian random variable \cite{Santner2003}.

% Basic notation
A \gls[hyper=false]{gp} is fully specified by its mean and covariance functions, instead mean vector and covariance matrix.
A \gls[hyper=false]{gp} $Y$ with a given mean function $m$ and covariance $K$ is denoted as
\begin{equation}
...
% Example
The shape of the random function drawn from a \gls[hyper=false]{gp} (or \emph{sampled path}) is characterized by its mean and covariance functions.
...
\begin{equation}
	\begin{split}
		m(\mathbf{x}_1) & = 0 \\
		K(\mathbf{x}, \mathbf{x}^*) & = \sigma^2 \exp{\left[-\frac{(\mathbf{x} - \mathbf{x}^*)^2}{2\theta^2}\right]}
	\end{split}
\label{eq:gp_example}
\end{equation}
