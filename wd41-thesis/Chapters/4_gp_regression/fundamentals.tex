...
Specifically, $\mathcal{Y}(\bm{x}), \, \text{for} \, \bm{x} \in \boldsymbol{X} \subseteq \mathbb{R}^D$ is a \emph{Gaussian process} if and only if for any choice from the finite set of input $\{\bm{x}_1, \bm{x}_2, \cdots, \bm{x}_L ; \, L \geq 1\}$, the random vector $\left[\mathcal{Y}(\mathbf{x}_1), \mathcal{Y}(\mathbf{x}_2), \cdots, \mathcal{Y}(\mathbf{x}_L)\right]$ is a multivariate Gaussian random variable \cite{Santner2003}.
...
As such, not all functions of the pair of inputs $\bm{x}_i, \bm{x}_j$ are \emph{valid} covariance functions, but only the ones that yield a valid variance-covariance matrix given by condition in Eq.~(\ref{eq:covariance_matrix}). 
...
\begin{equation}
	\begin{split}
		& [\mathcal{Y}(\bm{x}_i)] \sim \mathcal{N}_L(\boldsymbol{\mu}, \boldsymbol{\Sigma}) \quad ; \, i = 1, 2, \cdots, L \\
		& \boldsymbol{\mu} = [m(\bm{x}_1), m(\bm{x}_2), \cdots, m(\bm{x}_L)]^T \\
		  & \boldsymbol{\Sigma} = 
				\begin{pmatrix}
...
$\cdots, \bm{x}_L\} \in \boldsymbol{X} \subseteq \mathbb{R}^D$ with $L \geq 1$, 
and for $\bm{h} \in \mathbb{R}^D$ such that $\{(\bm{x}_1 + \bm{h}),$ 
$(\bm{x}_2 + \bm{h}), \cdots, (\bm{x}_L + \bm{h})\} \in \bm{X}$, the distribution of random vector $[\mathcal{Y}(\bm{x}_1 + \bm{h}), $
$\mathcal{Y}(\bm{x}_2 + \bm{h}), \cdots, \mathcal{Y}(\bm{x}_L + \bm{h})]$ is \emph{the same as} the distribution of random vector $[\mathcal{Y}(\bm{x}_1), \mathcal{Y}(\bm{x}_2), \cdots, \mathcal{Y}(\bm{x}_L)]$ \cite{Santner2003, Bachoc2013}.
...
The Gaussian kernel is parameterized by only a single \emph{hyper-pa}-\emph{rameter} $\theta$ that defines the characteristic length-scale of the process (or the \emph{range} parameter).
