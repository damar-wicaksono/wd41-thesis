...
while Appendix~\ref{app:gaussian_vector} gives more detail on Gaussian random vector (\gls[hyper=false]{mvn}).
...
To illustrate the notions of joint, marginal, and conditional distributions, an example of a bivariate random variable, a Gaussian random vector $\bm{\mathcal{Z}} = [\mathcal{Z}_1, \mathcal{Z}_2] \in \mathbb{R}^2$ is given.
...
% Closing Sentence
Gaussian stochastic generalizes this procedure beyond the $15$-variate Gaussian random variable to an arbitrary number of variables at arbitrary locations on the real line.
...
A \gls[hyper=false]{gp} $\mathcal{Y}(\bm{x})$ on $\boldsymbol{X} \subseteq \mathbb{R}^D$ with a given mean function $m$ and covariance $K$ is denoted as
\begin{equation}
	\mathcal{Y}(\bm{x}) \sim \mathcal{GP} \left(m(\bm{x}), K(\bm{x}, \bm{x}^*) \right)
\label{eq:gp_notation}
\end{equation}

% Mean Function
The mean function of a Gaussian process $\mathcal{Y}(\bm{x})$ is the function $m: \boldsymbol{X} \subseteq \mathbb{R}^D \mapsto \mathbb{R}$ defined as,
\marginpar{Mean function}
\begin{equation}
	m(\bm{x}) = \mathbb{E}[\mathcal{Y}(\bm{x})]
\label{eq:gp_mean_functions}
\end{equation}

% Covariance Function
The covariance function of a Gaussian process $\mathcal{Y}(\bm{x})$, on the other hand, is the function $K: (\boldsymbol{X} \subseteq \mathbb{R}^D) \times (\boldsymbol{X} \subseteq \mathbb{R}^D) \mapsto \mathbb{R}$ defined as,
\marginpar{Covariance Function}
\begin{equation}
	K(\bm{x}_i, \bm{x}_j) = \text{Cov}[\mathcal{Y}(\bm{x}_i), \mathcal{Y}(\bm{x}_j)]
\label{eq:gp_cov_functions}
\end{equation}
Notice that while the covariance function describes the covariance between pairs of random function values, 
it is defined only as a function of the two inputs, $\bm{x}_i$ and $\bm{x}_j$.
The covariance function is also sometimes referred to as the \emph{covariance kernel} function as it defines the elements of the covariance matrix (see example below).
Not all functions of the pair of inputs $\bm{x}_i, \bm{x}_j$ are \emph{valid} covariance functions, but only the ones that yield a valid variance-covariance matrix given by the condition in Eq.~(\ref{eq:covariance_matrix}). 
...
\begin{equation}
	K(\bm{x}_i, \bm{x}_i) = \text{Cov}[\mathcal{Y}(\bm{x}_i), \mathcal{Y}(\bm{x}_i)] = \mathbb{V}[\mathcal{Y}(\bm{x}_i)]
...
\begin{equation}
	\begin{split}
		& [\mathcal{Y}(\bm{x}_i)] \sim \mathcal{N}_L(\boldsymbol{\mu}, \boldsymbol{\Sigma}) \quad ; \, i = 1, 2, \ldots, L \\
		& \boldsymbol{\mu} = [m(\bm{x}_1), m(\bm{x}_2), \ldots, m(\bm{x}_L)]^T \\
		  & \boldsymbol{\Sigma} = 
				\begin{pmatrix}
					\mathbb{V}[\mathcal{Y}(\bm{x}_1)]  & \cdots & \text{Cov}[\mathcal{Y}(\bm{x}_1), \mathcal{Y}(\bm{x}_L)]\\
					\vdots	& \ddots & \vdots\\
					\text{Cov}[\mathcal{Y}(\bm{x}_L), \mathcal{Y}(\bm{x}_1)]  & \cdots  & \mathbb{V}[\mathcal{Y}(\bm{x}_L)]\\
			\end{pmatrix} \\
	\end{split}
\label{eq:gp_to_mvn}
\end{equation}
...
Examples of five realizations from the \gls[hyper=false]{gp} are shown in Fig~\ref{fig:plot_random_function_1}.
A realization of a \gls[hyper=false]{gp} on selected input locations is also called a \emph{sample path} of the process \cite{Santner2003},
a term which will be used interchangeably with the term realization of a \gls[hyper=false]{gp} (or a stochastic process in general).
\normdoublefigure[pos=tbhp,
                  mainlabel={fig:plot_random_function},
                  maincaption={Five realizations (sample paths) of a Gaussian process specified in Eq.~(\ref{eq:gp_example}) at $x_i = \{-2 + 0.2 \times i\}_{i=0}^{20}$. Shaded area indicates the area enveloped by twice standard deviation of the process (or $95\%$ probability region). In the right panel, the sample paths are drawn conditional on six observed values (cross symbols).},%
									mainshortcaption={Realizations of a Gaussian process, conditional and unconditional},
                  leftopt={width=0.425\textwidth},%width=0.45\textwidth},
                  leftlabel={fig:plot_random_function_1},
                  leftcaption={Unconditional},
                  %leftshortcaption={},%
                  rightopt={width=0.425\textwidth},%width=0.45\textwidth},
...
Suppose now that values of six variables are fully observed as follows $\{(x_i, y_i)\}_{i=1}^{6} = \{(-2.0, -0.75), (-1.2, 1.5), (-0.8, 2.75), (0.4, 3.75),$ 
$(1.2, -1.3), (1.8, -3.8)\}$.
...
Examples of five sample paths from such conditional distribution are shown in Fig.~\ref{fig:plot_random_function_2}.
...
A stochastic process $\mathcal{Y}(\circ)$ is called \emph{strictly/strongly stationary} if and only if for any finite set of inputs $\{\bm{x}_1, \bm{x}_2,$
$\ldots, \bm{x}_L\} \in \boldsymbol{X} \subseteq \mathbb{R}^D$ with $L \geq 1$, 
and for $\bm{h} \in \mathbb{R}^D$ such that $\{(\bm{x}_1 + \bm{h}),$ 
$(\bm{x}_2 + \bm{h}), \ldots, (\bm{x}_L + \bm{h})\} \in \bm{X}$, the distribution of random vector $[\mathcal{Y}(\bm{x}_1 + \bm{h}), $
$\mathcal{Y}(\bm{x}_2 + \bm{h}), \ldots, \mathcal{Y}(\bm{x}_L + \bm{h})]$ is \emph{the same as} the distribution of random vector $[\mathcal{Y}(\bm{x}_1), \mathcal{Y}(\bm{x}_2), \ldots, \mathcal{Y}(\bm{x}_L)]$ \cite{Santner2003, Bachoc2013}.
...
A stochastic process $\mathcal{Y}(\circ)$ is called \emph{weakly stationary} if and only if
...
\begin{equation}
	\text{Cov}[\mathcal{Y}(\bm{x}_i), \mathcal{Y}(\bm{x}_j)] = \text{Cov}[\mathcal{Y}(\bm{x}_i + \bm{h}), \mathcal{Y}(\bm{x}_j + \bm{h})] = K (\bm{x}_i - \bm{x}_j)
...
Additionally, following Eq.~(\ref{eq:stationary_covariance}),
the process variance can be defined as the covariance at zero distance or $K(0)$,
which is constant across input parameter space.
...
For instance, the stationarity assumption can be relaxed simply by considering a non-constant mean function as proposed in  Refs.~\cite{Marrel2008,Ginsbourger2009}, while keeping the covariance part stationary.
Another alternative is to consider multiple stationary covariance functions defined for each partitioned region of the whole input parameter space as proposed in Ref.~\cite{Gramacy2008}.
...
\begin{equation}
	K (\bm{x}_i, \bm{x}_j) = \sigma^2 R(\bm{x}_i, \bm{x}_j) 
	\label{eq:cov_function}
\end{equation}
where $R$, the correlation kernel function, is defined such that $\bm{x}_i, \bm{x}_j \in \boldsymbol{X} \subseteq \mathbb{R}^D \, \forall \, i, j $;
...
Later on, these $1$-dimensional functions are used to create a multidimensional kernel function $R(\bm{x}_i, \bm{x}_j)$ by means of tensor product.
...
The Gaussian kernel is parameterized by a single \emph{hyper-parameter} $\theta$ that defines the characteristic length-scale of the process (or the \emph{range} parameter).
...
The realization of the process, therefore, will exhibit more erratic behavior at short ranges as it allows for changes that are more abrupt over shorter distance and less dependent of the neighboring values.
...
That is, at any given point the Gaussian kernel is continuous and differentiable (see the neighborhood of the origin of Fig.~\ref{fig:plot_corrfun_gauss}).
...
though as mentioned in Ref.~\cite{Rasmussen2006} the overly smooth process can result in either physically unrealistic or numerically difficult situations (i.e., the resulting variance-covariance matrix is ill-conditioned for poorly selected design points).
...
\bigtriplefigure[pos=tbhp,
			           ...
			           maincaption={Examples of realizations from \gls[hyper=false]{gp} with Gaussian correlation kernel for three different values of range parameter. The plotting range of the $y$-axis for each panel are set to $\pm \, 3 \times \sigma$. Each process has the same process variance, $\sigma^2 = 1.0$},
...
%===================================================================
\subsubsection{Power-Exponential Kernel}\label{subsub:gp_powexp_cov}
%===================================================================

The Gaussian correlation kernel belongs to a wider class of $2$-param\-eter kernel function family called the \emph{power-exponential} kernel and is given by \cite{Roustant2012,Santner2003,Rasmussen2006},
\begin{equation}
...
\marginpar{Shape parameter $p$}
Specifically, the shape parameter $p$ determines the differentiability of the process at the origin \cite{Abrahamsen1997}.
Fig.~\ref{fig:plot_corrfun_powexp} shows the correlation value of power-exponential kernel with three different values of $p$ and $\theta$ as function of $L_1$ norm ($|\bm{x}_i - \bm{x}_j|$).
\bigtriplefigure[pos=tbhp,
...
Although, strictly speaking, only when $p = 2$ is the power-exponential correlation differentiable at the origin (thus guarantee the smoothness of the realization),
...
\normdoublefigure[pos=tbhp,
                  ...
                  maincaption={Mat\'ern kernels for two different range parameters $\theta$ and, for each, two different shape parameters $\nu$.},%
...
%----------------------------------------------------------------------
\subsection{Multidimensional Construction}\label{sub:gp_multidimension}
%----------------------------------------------------------------------
...
%---------------------------------------------------
\subsection{Process Variance}\label{sub:gp_variance}
%---------------------------------------------------
...
As shown, the visible features of the realizations remain very similar to each other.
What has changed, however, is the scale of the variation in the output space.  
\bigtriplefigure[pos=tbhp,
								 mainlabel={fig:plot_process_variance},
			           maincaption={Realizations of \gls[hyper=false]{gp} with Gaussian correlation kernel for three different values of process variance. The plotting range of the $y$-axis for each panel is set to $\pm \, 3 \times \sigma$.},
...
Fig.~\ref{fig:plot_mean_function_unconditional} shows several realizations drawn from three \glspl[hyper=false]{gp} having the same covariance kernel, but with three different mean functions.
As it can be seen, the process are centered differently for the three \glspl[hyper=false]{gp}. 
...
Indeed, it was argued in Ref.~\cite{Currin1991} that the use of either zero or constant mean in GP to model a process signifies a vague or the least informative prior to the unknown.
