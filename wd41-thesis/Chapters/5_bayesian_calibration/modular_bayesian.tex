...
This illustrates the two symptoms of overfitting the model parameter: the calibrated model parameter is \emph{biased} (i.e., with values inapplicable for prediction outside the calibration domain) and the distribution characterizing its uncertainty is \emph{degenerate} (i.e., increasingly sure on the wrong value with higher precision of the observed data).
...
Because the model parameters and the model bias are not fully identifiable according to Eq.~(\ref{eq:bc_true_simulation})\footnote{without further prior information, arbitrary $\hat{\bm{x}}_m$ fits the data perfectly well for arbitrary $\delta$. In other words, the two terms are \emph{confounds}.}, 
...
Furthermore, having a true value, such physical parameters would then be generally applicable to extrapolation outside the calibration domain.
...
Incorporating model bias term might help in alleviating the problem of overfitting, but a priori arbitrariness of the model bias term confounds with the model parameters itself, making the resulting calibrated model parameters more difficult to interpret \cite{Higdon2004}.
...
Ref.~\cite{Brynjarsdottir2014} went further by arguing that such model parameters (tuning) had limited scientific values and would not help in extrapolation domain.

This thesis takes a more conciliatory and pragmatic approach regarding this dichotomy: the distinction is rather irrelevant.
It is true that if the notion of true parameter values cannot be justified then the model parameters might not have strict physical meaning and may not be of interest in their own right.
And yet, in a complex physics-based simulator (where possible systematic bias cannot be excluded), many of these model parameters are being used in conditions different from the calibration domain, regardless of the conceptual distinction (e.g., Refs.~\cite{Arendt2012,USNRC2012}).
Thus, the calibration of model parameters based on the available experimental data should be aimed such that the simulator remains to applicable when it is applied outside its calibration domain\footnote{or more eloquently, the resulting uncertainty associated with calibrated model parameters is, in the words of Leamer \cite{Saltelli2006}:``...wide enough to be credible and the corresponding interval of inferences is narrow enough to be useful''.}.
The Bayesian framework, by allowing selection of prior uncertainties both for model parameters and for model bias which eventually results in the associated posterior uncertainties, accomodates this aim of calibration in a flexible manner.
