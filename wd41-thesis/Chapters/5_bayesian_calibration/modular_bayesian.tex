...
Regarding this dicothomy, the present thesis takes a more pragmatic approach: the distinction is rather irrelevant.
...
the uncertainties due to the specifications of the model bias, the observation error, and the simulator approximation up to their hyper-parametrization are taken into account (the so-called \emph{full probabilistic} treatment). 
...
Though the simulation method of Section~\ref{sec:bc_mcmc} tends to have less severe dependence on the problem dimension, more parameters often also increases the complexity of the problem that causes the method to converge slowly.
...
Indeed, this what was done in Chapter~\ref{ch:gp_metamodel} where the training of the metamodel (i.e., estimating the hyper-parameters through \glsfirst[hyper=false]{mle}) was separated from the calibration of the model parameters.
Afterward, the metamodel was only required to accurately predict the simulator outputs for arbitrary inputs (both model parameters and controllable inputs) within a predefined range through an independent validation step (using additional simulator runs).
...
Lastly, for the observation error, instead of  

% Summary, Closing and Inter-Section Connecting Paragraph Consequence
The modularization represents a compromise between having a full uncertainty analysis and having a more tractable formulation of the model parameters calibration problem \cite{Kennedy2001}.
Such a compromise then becomes part of the modeling decision in order to simplify a particular calibration problem:
...
and fixing hyper-parameters associated with the Gaussian distribution of the observation error $\bm{\Psi}_\epsilon$ at \emph{estimated} values implies that the uncertainty in the observation error is not taken into account completely.

As such, it is important in the simplified analysis to recognize what is being compromised above and to recognize properly the sources of uncertainty in the calibration at the level of metamodel (i.e., simulator approximation), model bias, and observation error in the first place.
Indeed as it was reported in Refs.~\cite{Kennedy2001,Liu2005,Bayarri2007,Arendt2012},
the effect of the additional sources of uncertainty at the level of the hyper-parameters are relatively minor on the calibration results.
