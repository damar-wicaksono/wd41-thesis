...
Because the model parameters and the model bias are not fully identifiable according to Eq.~(\ref{eq:bc_true_simulation}) (they are confounded), 
...
% Some philosophical issue, not true but aim for applicability
% Physical Parameter vs. Tuning Parameter, some philosophical issue
Finally, the discussions above also illustrate the distinction between a physical and a tuning parameter in the interpretation of a model parameter.
As argued in \cite{OHagan2013,Brynjarsdottir2014}, a physical parameter often have meaning outside the world described by the model such that the notion of.
In practice, however, considering the prevalence of physics-based simulator it is inappropriate to discuss the presence of true value\cite{Campbell2006,Brynjarsdottir2014}.

A more conciliatory and pragmatic approach to this is through a simple notion of applicability when discussing the uncertainty associated with the calibrated model parameters\footnote{That is, the resulting uncertainty associated with calibrated model parameters is, in the words of Leamer \cite{Saltelli2006}:``...wide enough to be credible and the corresponding interval of inferences is narrow enough to be useful''.}.
Regardless of this conceptual distinction, there are uncertainties associated with these parameters especially as the condition at which they are calibrated can be different from the intended use of the simulator (in which the parameters reside).
Thus, their calibration based on experimental data should be aimed such that the simulator remains applicable when it is applied outside its calibration domain.
