...
The vectors $\bm{y}_M(\circ, \boldsymbol{\lambda})$ and $\bm{y}_E(\circ, \boldsymbol{\lambda})$ for $\boldsymbol{\lambda} \in \boldsymbol{\Lambda}$ then refer to the model prediction and experimental data given by the element $\boldsymbol{\lambda}$ of set, respectively.
...
Both in the physical experiment and in the simulation their values are often varied either to investigate the system (or simulated system) behavior under the change or to find the setting that gives the best system performance.
