...
The last calibration scheme investigated was the \texttt{w/o Bias} scheme in which no model bias term was incorporated in the calibration process and the only sources of uncertainties was the one associated with the reported experimental data (see Section~\ref{subsub:bc_calibration_schemes}.
The results as presented in Fig.~\ref{fig:ch5_plot_ens_all_nodisc} showed a peculiar behavior.
A notable feature was that, for many model parameters, the nominal parameters values were found to be outside the $95\%$ posterior uncertainty range.
...
Therefore, if this results is to be believed then it also implies that the previous calibration results are highly implausible to simulate the reflood experiment of \gls[hyper=false]{feba};
and that there are (wildly) different model parameters that allow \gls[hyper=false]{trace} to simulate the experiment better.
...
The parameters \texttt{iafbWHT} and \texttt{tQuench}, for instance, were concentrated on both sides of their prior uncertainties;
while the parameters \texttt{dffbWHT} and \texttt{dffbVIHT} were concentrated on one side of their prior uncertainties.
...
In the following, to have a better understanding on the different calibration schemes and on the effect of considering model parameter correlations, 
...
This is supported by the sensitivity analysis conducted in the previous chapter where it was shown that the parameter \texttt{dffbVIHT} was important for the $CO$ output and became increasingly important for the $TC$ output at higher elevations.
...
This study suggests that a different reflood closure model adjustment might be required to properly simulate reflood in the upper part of dry assembly, in a low system pressure (here it is $2.1\,[bar])$ or with higher reflood rate (i.e., test Nos.~$214$ and~$222$).
