...
\begin{enumerate}
  ...
  \item Assume the vectors $\left(\mathbf{y}_{E,TC} - \mathbf{\hat{y}}_{M,TC}\right)$, $\left(\mathbf{y}_{E,DP} - \mathbf{\hat{y}}_{M,DP}\right)$, and\\ $\left(\mathbf{y}_{E,CO} - \mathbf{\hat{y}}_{M,CO}\right)$ to be realizations from stationary \glspl[hyper=false]{gp} on the observation layouts (i.e., as function time and space).
    Note that as the observation layouts comprise discrete points in time and space that the \glspl[hyper=false]{gp} collapse to multivariate Gaussian distributions.
  ...
  In the present analysis the values of the hyper-parameters estimated in the previous step are kept constant during the subsequent phase of calibration.
\end{enumerate}
...
By varying the parameters related to the boundary conditions in the construction of the term implies that the residual uncertainty of the experiment associated with with the variations of the boundary conditions is included in the model bias term.
In the \gls[hyper=false]{feba} experiment, the residual uncertainty was not directly observed as there was no replication of the experiment at the same controllable inputs.
According to the above formulation, the model bias term allows for such uncertainty based on \gls[hyper=false]{trace} simulation.
...
To investigate the implication of the different calibration schemes on \gls[hyper=false]{trace} predictions, simulation campaigns of \gls[hyper=false]{feba} test No. $216$ are conducted using samples from the posterior \glspl[hyper=false]{pdf}.
...
the simulations campaigns are extended to the $5$ additional \gls[hyper=false]{feba} tests.
...
\begin{equation}
  \text{Inf}(src, y) = \frac{|\pi_{\text{ign}, Y}| - |\pi_{\text{src},Y}|}{|\pi_{\text{ign},Y}|} = 1 - \frac{1}{2} \frac{UUB_Y - LUB_Y}{UUB_{Y, \text{max.}} - LUB_{Y, \text{min.}}}
...
\normdoublefigure[pos=tbhp,
                  ...
                  maincaption={Informatives of two different information sources. Wider uncertainty interval gives lower informativeness, and vice versa. Thus $\text{Inf}(src_1, y) > \text{Inf}(src_2, y)$},%
