...
Combining the terms of the above according to Eq.~(\ref{eq:bc_observation_simulation_true}) gives similar formulation as Eq.~\ref{eq:bc_data_model_gaussian}. 
That is, the data generating model for the $TC$ data is given as follow,
\begin{equation}
	\begin{split}
		& \bm{\mathcal{Y}}_{E,TC} | \mathbf{x}_m \sim \mathcal{N} (\boldsymbol{\mu}_{TC} (\bm{x}_m), \Sigma_{TC} (\bm{x}_m)) \\
		& \boldsymbol{\mu}_{TC} (\bm{x}_m) = \boldsymbol{\mu}_{M,TC} (\bm{x}_m) + \mathbf{m}_{\delta,TC} \\
...
The mean vector $\boldsymbol{\mu}_{TC}$, in turn, consists of the mean vector of the \gls[hyper=false]{gp} metamodel prediction $\boldsymbol{\mu}_{M,TC}$ (Eq.~(\ref{eq:p_variate_metamodel_tc}));
...
Furthermore, the $LUB_{\text{min.}}$ and $UUB_{\text{max.}}$ are taken to be the the $2.5$-th and $97.5$-th percentiles of the prior prediction distribution and the reference value $y_\text{ref.}$ of the each posterior prediction distribution is taken to be its median value.
