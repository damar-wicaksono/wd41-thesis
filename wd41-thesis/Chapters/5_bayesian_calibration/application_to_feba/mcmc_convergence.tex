...
To check the convergence of the results of an ensemble samplers, it is a common practice to investigate the running statistics of the ensemble (i.e., statistics over all walkers per iteration) instead of the individual walkers \cite{Foreman-Mackey2013,Akeret2013}.
...
Although the length of the burn-in period can be inferred directly from the plots in Fig.~\ref{fig:ch5_plot_ens_stat_mcmc}, a more rigorous criteria can be obtained via the autocorrelation time of the running statistics.
...
To obtain a set of independent samples, the remaining samples are thinned on the basis of the re-estimated autocorrelation time.
Using (twice, Eq.~(\ref{eq:ch5_markov_chain_estimator_variance})) the largest autocorrelation time for thinning, $32'000$ posterior samples out of $1'240'000$ post-burn-in samples can be considered independent and readily used for forward uncertainty propagation.
Though it results in a much smaller sample size, the associated relative statistical error (based on the number of independent samples) for the model parameters estimates is at most $0.6\%$ (Eq.~(\ref{eq:ch5_markov_chain_relative_error_ensemble})).
