...
The chain is used to estimate the expectation of a function $f$ under $\pi_t$ such that $\hat{f} = 1/I \sum_i f(\bm{\mathcal{X}}^{(i)})$, and the asymptotic error of the estimator follows as
\begin{equation*}
...
For instance, $10'000$ \gls[hyper=false]{mcmc} iterations are relatively long for a single particle chain having an autocorrelation time of $5$ ($\epsilon \approx 3\%)$, but much shorter in comparison for a chain having an autocorrelation time of $100$ ($\epsilon \approx 14\%)$.
...
% On thinning
The notion of independent samples (or effective sample size) mentioned above are coupled with the practice of \emph{thinning} or sub-sampling of the \gls[hyper=false]{mcmc} samples \cite{Geyer1992}.
...
Often, the integer $k$ is chosen to be close to, or at least the same order of magnitude as, the twice the autocorrelation time $\tau_{\text{int},f}$.
...
At the same time, there can be much smaller number of samples left after thinning that often worsen the accuracy of the estimator.
...
By \emph{burn-in} the chain, it is meant that the samples of that period are discarded from further analysis.
...
Once more, in practice, a conservative multiples of the estimated autocorrelation time $\tau_\text{exp}$, ranging from $5$ \cite{Foreman-Mackey2013} to $20$ \cite{Sokal1997}, is used to determine the total number of iterations to discard.
...
% Why there's burn-in
The initial transient is mainly due to the selection of values with low posterior probability to initialize the chain.
