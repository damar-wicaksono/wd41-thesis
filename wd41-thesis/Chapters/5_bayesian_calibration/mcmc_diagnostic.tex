...
First, the use of a Markov chain of length $I$ to estimate the expectation of $f$ inflates the estimator variance by factor $2 \tau_{\text{int},f}$.
...
That is, the minimum length of the chain can be determined such that the statistical error of an estimator $\mathbb{V}[\hat{f}]^{0.5}$ is smaller than a target fraction $\epsilon$ of the true standard deviation $\mathbb{V}[f]^{0.5}$ \cite{Akeret2013},
\begin{equation}
...
For a single particle \gls[hyper=false]{mcmc} this number is also the number of \gls[hyper=false]{mcmc} iterations.
...
But, this is often the case for an infinitely long chain \cite{Sokal1997}.
...
In a simple problem, perhaps it can be straightforward to determine a value that lies on high probability region of the posterior.
