...
\end{equation}
Direct estimation of the autocorrelation time using the above estimator is usually unstable due to the statistical noise associated with large $t$.
For a large $t$, the data of iterations that are far separated becomes sparser for a reliable estimate of the autocovariance function value. 
A ``windowing'' method is proposed by \cite{Sokal1997} to stabilize the estimation and is implemented in the routine \texttt{ACOR} \cite{Goodman2009,Foreman-Mackey2014} used in this thesis.
...
\marginpar{Determining iterations to discard}
In practice, a conservative multiples of the exponential autocorrelation time $\tau_\text{exp}$, ranging from $5$ \cite{Foreman-Mackey2013} to $10$ \cite{Sokal1997}, is used to determine the total number of iterations to discard.
...
This tuning, however, also entails additional calculation and optimization.
