...
The chain is used to estimate the expectation of a function $f$ under $\pi_t$ such that $\hat{f} = 1/I \sum_i f(\bm{\mathcal{X}}^{(i)})$, as the asymptotic error of the estimator follows as
\begin{equation*}
...
At the same time, there can be much smaller number of samples after thinning that often worsen the accuracy of the estimator.
 
Thinning can be justified in the case expensive ``post-processing'' of the \gls[hyper=false]{mcmc} samples.
