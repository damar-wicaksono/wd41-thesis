...
The method was also able to reduce the size of the problem by screening out noninfluential inputs.
...
The framework casts the calibration problem as a statistical inverse problem where the initial (\emph{prior}) uncertainties of the inputs are updated based on available observed data.
...
The inputs uncertainties are then coherently updated via the Bayes' theorem resulting in an updated (\emph{posterior}) probability density.
...
To do so, different types of experimental data (i.e., clad temperature, pressure drop, and liquid carryover) are used and their ability to constrain the prior range is investigated.
...
Finally, Section~\ref{sec:bc_chapter_summary} concludes the chapter.
...
\input{Chapters/5_bayesian_calibration/mcmc_diagnostic}
