...
The goal of the present chapter was to quantify the uncertainty associated with model parameters a posteriori based on observed/experimental data.
...
Some practical aspects of analyzing the resulting samples were provided.
...
This posterior correlation structure \emph{and} the posterior range were specific to the calibration data (here it was based on one \gls[hyper=false]{feba} test run).
...
However, if the data was deemed not sufficiently large or comprehensive enough then care should be taken to avoid overfitting.
It was shown that by excluding one of the correlated parameters has allowed for a more precise estimation of the other parameters that were previously correlated.
...
The calibration scheme with model bias term and incorporated all types of outputs was able to constrain the prior uncertainties of the model parameters while keeping the nominal TRACE parameters values within the posterior uncertainty interval.
...
This illustrated the value of the incorporating the model bias term in order to avoid overfiting especially considering the fact that the calibration conducted here was based on one test run. 
