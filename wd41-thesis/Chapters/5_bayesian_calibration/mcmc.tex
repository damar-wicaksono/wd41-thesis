...
% Grid Approach
A straightforward approach to generate samples from a given multivariate density is by first discretizing the input parameter space of the density function and evaluate the density at the discretized points.
\marginpar{Discretized grid approach}
Supposed the domain of the density has been discretized uniformly in each dimension with a level $\Delta$ resulting in $\{x_i\}_{i=1}^{I}$ with $I$ the number of discretized points, such that.
At the discretized levels, each value of $\bm{x}_i$ has a probability $p(\bm{x}_i) = p^*(\bm{x}_i) / \sum_i p^*(\bm{x}_i)$. 
Afterward, sample values can be generated using probability proportional to the values of the density at the discretized points. 

Fig.~\ref{fig:ch5_plot_gumbel_sample_grid} illustrates this procedure for the example given above.
First, the input parameter space is windowed in $\mathbf{X}\in[-25,25]^2$ before being discretized in $\Delta = 40$ levels.
...
Together they constitute a complete discrete probability distribution.
Generating samples from such a discrete probability distribution is straightforward in modern computing environment.
Fig.~\ref{fig:ch5_plot_gumbel_sample_grid_1} shows $5'000$ samples generated from the discrete distribution.
Darker points indicate that the values have been sampled multiple times following the actual underlying \gls[hyper=false]{pdf} (the contour of the analytical joint density is overlaid to serve as a guide). 
...
\bigtriplefigure[pos=tbhp,
			           ...
			           maincaption={Sampling from a multivariate density by discretizing the input parameter space in grids. The input parameter space is windowed and discretized into $\Delta = 40$ levels. The density is then evaluated at the discretized points. (Left) $5'000$ samples are generated following the resulting discrete probability distribution; (Center and Right) The histograms approximately follows the shape of the analytical marginal density.},
...
The main issue with the discretized grid approach, straightforward as it is,
...
the likelihood function inside the posterior generally can be a complex function.
In consequence, a very fine discretization level might be required to appropriately capture the function at important regions which are unknown a priori.
