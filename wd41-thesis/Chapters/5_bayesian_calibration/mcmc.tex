...
As shown, because the proposed moves can jump from one side of the parameter space to another,
...
In this case, any proposed move around the previously accepted state would almost always be accepted and the chain traverses the parameter space very slowly Fig.~\ref{fig:ch5_plot_illustrate_markov_chain_small_step_1}.
...
As such, some sort of tuning is required regarding the proposal distribution for it to be optimal with respect to a chosen measure.
...
% Entry to AIES
Instead of delving into any of the particular improvements on the \gls[hyper=false]{mh} algorithm (such as through the adaptive schemes)\footnote{Refs. provide overviews of the adaptive \gls[hyper=false]{mc} algorithms, while are examples of adaptive algorithms.},
...
The main ideas of the above (i.e., proposed move and its acceptance probability), however,
