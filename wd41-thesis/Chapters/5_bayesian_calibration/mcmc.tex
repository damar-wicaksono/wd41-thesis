...
\end{equation}
where $\mathbf{D}$ is the set of all possible densities on $\mathbf{X}$.
It implies that regardless of the starting density, some transition kernels will converge to a unique stationary density.
This in turn, is of practical importance when the transition kernel in a \gls[hyper=false]{mcmc} algorithm was design such that the given target density is the stationary density.
...
Another important results in relation to the application of Markov chain for \gls[hyper=false]{mc}
\marginpar{Ergodic theorem}
simulation is the \emph{ergodic theorem} which states that for a given realization $\{\bm{x}^{(i)}\}$ of a Markov chain $\{\bm{\mathcal{X}}^{(i)}\}$ with a stationary density $\pi^*$ and for any function $f: \mathbf{X} \mapsto \mathbb{R}_{< \infty}$, it holds that
\begin{equation}
	\lim_{I \rightarrow \infty} \frac{1}{I} \sum_{i = 1}^{I} f(\bm{x}^{(i)}) = \mathbb{E}_{\pi^*}[f(\bm{\mathcal{X}})] = \int_\mathbf{X} f(\bm{x}) \pi^*(\bm{x}) d\bm{x} 
...
This asymptotic result provides a justification for using a samples generated from a Markov chain (of certain conditions) for estimating integral in a more conventional \gls[hyper=false]{mc} simulation.
