...
The proposal probability density is responsible for generating a proposal or candidate move $\bm{x}^*$ for the Markov chain at each iteration and it is in general a density conditional on the previous state.
...
and $q(\bm{x}^{(i-1)} | \bm{x}^*)$ ($q(\bm{x}^* | \bm{x}^{(i-1)})$) are the value of the proposal density at the previous (proposed) state conditional on the value of the proposed (previous) state.
...
If the proposed move is accepted it becomes the current state of the chain,
...
Consequently, any proposed move that yields an ``improvement'' on the target density evaluation will be accepted, otherwise it will only be accepted according to its acceptance probability. 
...
For this example, the proposal distribution is chosen to be a bivariate normal with the variance of $2.0$ for both $x_1$ and $x_2$ and zero correlation.
...
a proposed move is drawn from the bivariate normal distribution (centered at the origin).
The proposed move brings the state closer to the center of the target density, thus it is accepted (Fig.~\ref{fig:ch5_plot_illustrate_markov_chain_iteration_2}).
A new proposal move is generated from the bivariate normal centered at the newly accepted move.
This time, because the proposed move moves farther away from the center of the target density, it is rejected.
...
Note that this kind of proposed move will not always be rejected outright but is subject to chance based on the acceptance probability.
...
In the long run, the chain will spend more time in the regions where the density are high and less time where the density low\footnote{The goal of an \gls[hyper=false]{mcmc} algorithm is not, on the other hand, to obtain the parameter value which maximizes the target distribution. It seeks to explore the parameter space in proportion to value of the density function.}.
Therefore, the resulting samples generated by the chain will be distributed according to the target distribution.
Figs.~\ref{fig:ch5_plot_illustrate_markov_chain_1} and~\ref{fig:ch5_plot_illustrate_markov_chain_1} are the \emph{trace} plots for the chain after $50'000$ iterations.
\marginpar{trace plot}
A trace plot shows the evolution of the chain during the iterations and it is the first graphical diagnostic tool to spot any possible problem of convergence a \gls[hyper=false]{mcmc} albeit rather unrigorously \cite{Robert2010}.
...
and $x_2$ has a relatively larger dispersion than $x_1$ (which are indeed the case from the contour plot).
\bigtriplefigure[pos=tbhp,
								 mainlabel={fig:ch5_plot_illustrate_markov_chain},
			           maincaption={Illustration of Markov Chain simulation to generate samples from a target density given in the example. (Left) The chain traverses the parameter space. At each iteration, a move is proposed and accepted in probabilistic manner, as it was illustrated in Fig.~\ref{fig:ch5_plot_illustrate_markov_chain_iteration_1}. (Center and Right) The trace plots for each parameter.},
...
The whole point of generating samples via \gls[hyper=false]{mcmc} simulation is exactly because such  arbitrary high-dimensional distributions are hard to characterize (either as marginal, bivariate joint, etc.).
\bigtriplefigure[pos=tbhp,
...
The example above also indicates the possible impact of the choice of proposal distribution on the performance of \gls[hyper=false]{mh} algorithm.
...
% Different Samplers and improvements out there

% Entry to AIES
Instead of delving into any of the particular improvements on the \gls[hyper=false]{mh} algorithm (such as through adaptive schemes),
this thesis adopted a relatively new \gls[hyper=false]{mcmc} algorithm based on an \emph{ensemble} Markov chain.
The algorithm has the potential of requiring minimal tuning to a particular problem.
The main ideas of the above (i.e., proposed move and acceptance probability), however,
...
% Introductory Paragraph
%The points is for the chain to spend time in the parameter space in proportion to its probability.
...
\bigtriplefigure[pos=tbhp,
			           ...
			           maincaption={Illustration of a \emph{stretch move} update for a single walker in a $2$-dimensional parameter space.},
			           mainshortcaption={Illustration of a \emph{stretch move} update for a single walker in a $2$-dimensional parameter space.},%
			           leftopt={width=0.30\textwidth},
...
\begin{algorithm}
\caption[Affine-Invariance Ensemble Sampler (Strectch-Move)]{Affine-Invariant Ensemble Sampler (Stretch Move)\\ Generate samples from $p(\bm{x}) \propto p^*(\bm{x})$ using $L$ walkers in $I$ iterations}
\label{alg:aies}
\begin{algorithmic}
  \REQUIRE $T > 0$, $L \geq T + 1$, $p^*(\bm{x})$, and $q (z)$
		\STATE $\vec{\bm{x}}^{(0)} = [\bm{x}^{(0)}_1, \ldots, \bm{x}^{(0)}_L] \leftarrow \vec{\bm{x}}_0 \, ; \, \forall \vec{\bm{x}}_0 \in \mathbf{X}^L$
		\FOR{$t=1$ to $T$}
			\FOR{$l=1$ to $L$}
				\STATE pick randomly $\bm{x}_j$ from $\vec{\bm{x}}^{(t-1)}_{\sim l}$
				\STATE sample $z$ from $q(z)$
				\STATE $\bm{x}^* \leftarrow \bm{x}_j + z (\bm{x}^{(t-1)}_l - \bm{x}_j)$
				\STATE $\alpha \leftarrow \text{min} \left(\frac{p^*(\bm{x}^*)}{p^*(\bm{x}^{(t+1)}_l)} \times z^{D-1}, 1.0\right)$
			\STATE sample $u$ from $\mathcal{U}[0,1]$
			\IF{$u < \alpha$}
				\STATE $\bm{x}^{(t)}_l \leftarrow \bm{x}^*$
			\ELSE
				\STATE $\bm{x}^{(t)}_l \leftarrow \bm{x}^{(t-1)}_l$
			\ENDIF
			\ENDFOR
		\ENDFOR
\end{algorithmic}
