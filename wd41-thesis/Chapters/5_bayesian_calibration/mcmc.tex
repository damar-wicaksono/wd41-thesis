...
For this example, the proposal distribution is chosen to be an independent bivariate normal with the variance of $2.0$ for both $x_1$ and $x_2$.
...
Note that this kind of proposed move will not always be rejected outright but is subjected to chance based on the acceptance probability.
...
These iterations are then repeated for a total of $50'000$ times yielding $50'000$ samples from the target distribution.
Fig.~\ref{fig:ch5_plot_illustrate_markov_chain}
\bigtriplefigure[pos=tbhp,
...
remain central in constructing an ensemble Markov chain.
