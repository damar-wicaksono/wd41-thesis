...
in an application setting, the model parameters uncertainties are often propagated through the simulation model to obtain the uncertainty in the prediction.
...
This uncertainty can also be summarized with various statistics and, as before, many of these involve integration.
For instance, the variance of the output:
\begin{equation*}
...
generating samples from an arbitrary \gls[hyper=false]{pdf} is a general problem.
As such, in the following, a generic notation for an arbitrary \gls[hyper=false]{pdf} $p(\bm{x})$ is used instead of $p(\bm{x}\,|\,\mathbf{y})$.
...
In one dimension, the most common way of generating samples from a given density is by inverse transform sampling coupled with a random number generator.
...
First, the state space is windowed in $\mathbf{X}\in[-25,25]^2$ before being discretized in $\Delta = 50$ levels.
...
\begin{itemize}
  ...
  \end{equation}
	...
	\begin{equation}
    \mathbb{P}(\bm{\mathcal{X}}^{(i)} \in \mathbf{A}\,|\,\bm{\mathcal{X}}^{(i-1)} = \bm{x}) = \int_\mathbf{A} T(\bm{x}, \bm{x}') \, d\bm{x}' \, ; \, \mathbf{A} \subseteq \mathbf{X}
...
Formally, for $\mathbf{A} \subseteq \mathbf{X}$,
\begin{equation}
...
This, in essence, is the objective of \gls[hyper=false]{mcmc} algorithms as defined in Ref.~\cite{Robert2004}.
...
It also indicates that $x_1$ are centered differently than $x_2$, and $x_2$ has a relatively larger dispersion than $x_1$.
\bigtriplefigure[pos=tbhp,
...
But this convergence might not be attained for a Markov chain of a practical length.
\normdoublefigure[pos=tbhp,
...
Through an affine transformation (to be defined further below),
...
\bigtriplefigure[pos=tbhp,
			           ...
			           midcaption={A complementary walk\-er is randomly selected},
...
\normdoublefigure[pos=tbhp,
                  ...
                  maincaption={Trace plots of individual walkers and the running mean and standard deviation for $x_1$.},%
									mainshortcaption={Trace plots of individual walkers and the running mean and standard deviation for $x_1$.},
...
The \gls[hyper=false]{aies} algorithm requires minimal tuning to generate samples from an arbitrary target distribution.
...
The algorithm was also recently reported to badly scale with the number of dimensions and to eventually fail in a high dimension ($D \geq 50$) either by a very slow convergence or, more importantly, by a biased convergence (i.e., convergence to a wrong value) \cite{Carpenter2017,Huijser2015}.
...
For instance, the ``initialization phase'' of the \gls[hyper=false]{aies} algorithm is an indication of a particular lack of convergence to stationarity in the initial part of the chain.
It needs to be detected and removed lest it would bias the estimation using its samples.
...
Afterward,
the main question is on how long the chain needs to be to obtain a small enough statistical error on the \gls[hyper=false]{mcmc} estimate.
