...
The formulation of the Bayesian calibration of a computer model presented above results in a joint posterior \gls[hyper=false]{pdf} for all the parameters involved in the resulting probability model $p(\bm{x}\,|\,\mathbf{y})$ (Section~\ref{sub:bc_modular_posterior}).
...
\end{equation*}
where $\bm{x} = \{x_d, \bm{x}_{\sim d}\}$ and the integrations are carried out over the domain $\mathbf{X}$.
...
\begin{equation}
	C \; \int p(\bm{x}) d\bm{x} = \int p^*(\bm{x}) d\bm{x}  \Leftrightarrow  C \; (1.0) = \int p^*(\bm{x}) d\bm{x}
\label{eq:bc_prop_const}
\end{equation}
Carrying out the integration of the above can be problematic in its own right especially for a highly multidimensional function (such as the normalizing constant of a posterior density given in Eq.~(\ref{eq:bc_normalizing_constant})).
...
consider the following unnormalized bivariate \gls[hyper=false]{pdf} parametrized by location parameters $\mu_1, \mu_2$ and scale parameters $\sigma_1, \sigma_2$ \cite{Balakrishnan2014}:
\begin{equation}
	\begin{split}
...
Although there is good idea where this typical set is located for the example above (the region around the center of the density),
in an arbitrary high-dimensional \gls[hyper=false]{pdf} (such as a posterior density) there will not be\footnote{And in fact, the region around the mode becomes less typical in high-dimension. Moving away from the mode of the density reduce its value while increasing the amount of volume.
In a high-dimensional case, the region of space around the mode contributes less and less mass in comparison to other part of the state space and thus becoming less typical. This is yet another example of the curse of dimensionality.}.
...
At the same time, if such samples can be obtained then the performance of \gls[hyper=false]{mc} integration in high dimension would be potentially be more efficient, requiring less function evaluations for the same accuracy level, to that of the grid approach\footnote{deterministic numerical integration is either based on or and improvement of a discretized grid approach.}.
...
\begin{itemize}
  ...
  \begin{equation}
    \begin{split}
      & T(\bm{x}, \bm{x}') \geq 0, \,\,\, \forall x, \bm{x}' \in \mathbf{X} \\
...
The distribution of $\bm{\mathcal{X}}^{(i)}$ due to the the transition from the previous iteration $\bm{\mathcal{X}}^{(i-1)}$, is given by the transition probability kernel $T$ operated on the density function of the previous iteration $\pi^{(i-1)}$ such that,
\begin{equation}
...
Under certain conditions (i.e., irreducible, aperiodic, and Harris recurrent~--~See Appendix~\ref{app:markov_chain}). for the transition kernel $T$, a stationary density $\pi^*$ exists, it is unique and is the \emph{limiting density} of the stochastic process, such that
\begin{equation}
...
it is the property of the sample path\footnote{Recall that from Chapter~\ref{ch:gsa}, a sample path is a realization of a stochastic process and in this case, a realization of a Markov chain.} of a finite size, $\{\bm{x}^{(i)}\}_{i=1}^{I}$, that matters when conducting an actual \gls[hyper=false]{mcmc} simulation \cite{Robert2004}.
By adding additional condition on T\footnote{Namely, irreducible, aperiodic, Harris recurrent, \emph{and} geometrically ergodic. Thus the chain is called (geometrically) ergodic\label{foot:bc_ergodicity}.}, another theorem states that for a given realization $\{\bm{x}^{(i)}\}_{i=1}^I$ of a Markov chain with stationary density $\pi^*$ and for function $f: \mathbf{X} \mapsto \mathbb{R}$ the following holds,
\begin{equation}
...
Finally, by imposing the conditions\footnote{Idem.}, the stationary distribution of the chain $\pi^*$ converges to the target distribution $\pi_t$.
...
Notice from the ratio, that the proportionality constant in Eq.~(\ref{eq:bc_prop_const}) cancels out and only the unnormalized density up to that constant $p^*$ is required.
As such, the potentially difficult multidimensional integral in Eq.~(\ref{eq:bc_prop_const}) has been dispensed with by this formulation.
...
Fig.~\ref{fig:ch5_plot_illustrate_markov_chain_small_step} shows the behavior of the chain in the case of too small variance parameter for the proposal distribution, in comparison to the scale of the target density.
...
The main ideas presented the above (i.e., proposal move and its acceptance probability), however,
...
Its main motivation is exactly where the previous section left off: the difficulty in tuning or adapting \gls[hyper=false]{mh} algorithm to make them applicable to wide class of target distribution.
...
Rigorous proof of stationarity is difficult to obtain in practice and instead, analysis is often based on empirical diagnostic that can only indicate a lack of stationarity.
Afterward,
