...
In fact, this ``region of space where it matters'' is related to the mathematical notion of \emph{typical set} of a distribution \cite{Mackay2005}.
...
in an arbitrary high-dimensional \gls[hyper=false]{pdf} (such as a posterior density) there is not\footnote{And in fact, the region around the mode becomes less typical in high-dimension. Moving away from the mode of the density reduce the density value while increasing the amount of volume.
...
Appendix~\ref{app:markov_chain} provides the definitions and illustrations for some of these concepts for a discrete state Markov chain where matrix notation and graphical representation are applicable.
Finally,
...
\begin{itemize}
  \item The \emph{state space} $\mathbf{X} \subseteq \mathbb{R}^D$, the values in which the random variables $\bm{\mathcal{X}}^{(i)}$ take on.
	
  \item The \emph{initial distribution} of $\bm{\mathcal{X}}^{(0)}$, given by the density $\pi^{(0)} = \pi^{(0)}(\bm{x})$.
	
	\item The \emph{transition probability kernel} $T(\bm{x}, \bm{x}')$ a function $T: \mathbf{X} \times \mathbf{X} \mapsto \mathbb{R}$.
...
The distribution of $\bm{\mathcal{X}}^{(i)}$ due to the the transition from the previous iteration $\bm{\mathcal{X}}^{(i-1)}$, are given by the transition probability kernel $T$ and the density function of the previous iteration $\pi^{(i-1)}$.
\begin{equation}
...
The rightmost definition signifies that the integration of the density function with the transition kernel is written as an operator on the density function $\pi^{(i-1)}$ resulting in  $\pi^{(i)}$ (cite Stracksy).
...
Central to the application of Markov chain for generating samples from an arbitrary probability density is the notion of stationary distribution of a Markov chain.
