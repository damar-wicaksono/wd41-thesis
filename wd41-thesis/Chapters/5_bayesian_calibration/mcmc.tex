...
% Typical set
Furthermore, many of the evaluations are seemingly wasteful because they are located far away from the center of the distributions; they are not part of the \emph{typical set} of the distribution.
This is especially important when evaluating integrals with respect to this density,
where many points in the discretized density potentially contribute close to nothing in the integration\footnote{Strictly speaking, a \emph{typical set} is a mathematical notion \cite{Mackay2005}. Loosely speaking, however, it can be thought of as the region of state space where the probability mass (density times volume) is concentrated. Moving away from the mode of the density reduce the density value while increasing the amount of volume.}.
...
% Add something about being directed toward typical set of the distribution
The
...
% Explain continuous state in relation with discrete state
Then,
