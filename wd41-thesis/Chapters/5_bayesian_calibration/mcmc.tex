...
The Metropolis-Hastings algorithms and its various extensions remains the most universal class of algorithms to generate such Markov chains \cite{Robert2010}. 
...
\begin{algorithm}
\caption[Metropolis-Hastings Algorithm]{Metropolis-Hastings Algorithm \\ Generate samples from $p(\bm{x}) \propto p^*(\bm{x})$ given proposal density $q (\bm{x})$ in $I$ iterations}
\label{alg:metropolis_hastings}
\begin{algorithmic}
  \REQUIRE $I > 0$, $p^*(\bm{x})$, and $q (\bm{x})$
