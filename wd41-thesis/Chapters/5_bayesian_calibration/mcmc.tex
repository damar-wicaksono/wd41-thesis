...
At the same time, if such samples can be obtained then the performance of \gls[hyper=false]{mc} integration in high dimension would be superior (requires less function evaluations for the same accuracy level) to that of the discretized grid approach\footnote{deterministic numerical integration is either based on or and improvement of a discretized grid approach.}.
...
The distribution of $\bm{\mathcal{X}}^{(i)}$ due to the the transition from the previous iteration $\bm{\mathcal{X}}^{(i-1)}$, is given by the transition probability kernel $T$ operated on the density function of the previous iteration $\pi^{(i-1)}$ as follows,
\begin{equation}
...
Under certain conditions\footnote{Specifically: irreducible, aperiodic, and Harris recurrent (see Appendix~\ref{app:markov_chain}).} for the transition kernel $T$, a stationary density $\pi^*$ exists, it is unique and is the \emph{limiting density} of the stochastic process, such that
\begin{equation}
...
Notice from the ratio, that the proportionality constant in Eq.~(\ref{eq:bc_prop_const}) cancels out and only the density up to that constant $p^*$ is required.
