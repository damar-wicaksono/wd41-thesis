...
and $q(\bm{x}^{(i-1)} | \bm{x}^*)$ ($q(\bm{x}^* | \bm{x}^{(i-1)})$) are the value of the proposal density at the previous state (proposed state) conditional on the value of the proposed state (previous state).
Note from the ratio, the proportionality constant in Eq.~(\ref{eq:bc_prop_const}) cancels out and only the density up to that constant $p^*$ is required (Eq.~(\ref{eq:bc_prop_post})). 
The acceptance probability is formulated to satisfy the \emph{detailed balance condition} (Appendix~\ref{app:markov_chain}) for any valid proposal probability distribution which, in turn, guarantees the stationarity of the process \cite{Chib1995} .
...
\marginpar{Random walk Metropolis-Hastings}
As a result the terms associated with the proposal density in the acceptance probability cancel each other.
Consequently, any proposal move that yields an ``improvement'' on the target density evaluation will be accepted, otherwise it will be only be accepted according to the acceptance probability. 
This particularly simple \gls[hyper=false]{mh} algorithm results in \emph{random walk} Markov chain
and as such it is termed the \emph{random walk} \gls[hyper=false]{mh}\cite{Robert2010}.  

% Application on the Simple Running Example
To illustrate the application of the \gls[hyper=false]{mh}, particularly the random walk \gls[hyper=false]{mh}, for generating samples from an arbitrary target distribution,
consider again the sample in Eq.~(\ref{eq:bc_unnormalized_gumbel}) used before.
\marginpar{Random walk Metropolis-Hastings algorithm, illustrated}
For this example, the proposal distribution is chosen to be the bivariate normal $\mathcal{N}_2(\bm{0.0}, \bm{2.0})$ with identical mean ($\bm{0.0}$) and variance ($\bm{2.0}$) for both $x_1$ and $x_2$.
...
% Entry to AIES
% Instead of delving into some particular improvements on the Metropolis,
% this thesis adopted a relatively new algorithm based on ensemble sampler
% The main idea introduced above hold though
