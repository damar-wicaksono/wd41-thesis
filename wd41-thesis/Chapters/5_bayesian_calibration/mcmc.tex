...
Additionally, sampling algorithms exist for several multivariate densities due to their special properties (e.g., generating sample from a multivariate normal density in Appendix~\ref{app:mvn_sampling}).
...
the cost of numerically integrating such function might itself be very costly due to the high dimension.
...
At the discretized levels, the probability for each value of $\bm{x}_i$ is approximated by $p(\bm{x}_i) = p^*(\bm{x}_i) / \sum_i p^*(\bm{x}_i)$\footnote{strictly speaking, each density value has to be multiplied by the hypervolume of the grid to obtain the probability mass, but the term cancels out in $p$.)}.
