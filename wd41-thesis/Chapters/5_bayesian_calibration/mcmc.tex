...
\begin{algorithm}
\caption[Affine-Invariance Ensemble Sampler (Strectch-Move)]{Affine-Invariant Ensemble Sampler (Stretch Move)\\ Generate $R = L * (T + 1)$ samples from $p(\bm{x}) \propto p^*(\bm{x})$}
\label{alg:aies}
\begin{algorithmic}
  \REQUIRE $T > 0$, $L \geq T + 1$, $p^*(\bm{x})$, and $q (z)$
		...
		\FOR{$t=1$ to $T$}
			\FOR{$l=1$ to $L$}
				...
				\STATE $\alpha \leftarrow \text{min} \left(\frac{p^*(\bm{x}^*)}{p^*(\bm{x}_l)} \times z^{D-1}, 1.0\right)$
			\STATE sample $u$ from $\mathcal{U}[0,1]$
			\IF{$u < \alpha$}
