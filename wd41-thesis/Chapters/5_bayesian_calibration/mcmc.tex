...
The theorem implies that for a long enough chain (assuming that the stationarity has been attained), the sample path $\{\bm{x}^{(i)}\}_{i=1}^I$ can be used to estimate the function $f$ integrated along the target \gls[hyper=false]{pdf} $\pi^*$ (e.g., the \glspl[hyper=false]{qoi} in the opening of this section) through the the sample mean of $f$ evaluated at the points of the sample path.
...
Notice from the ratio, that the proportionality constant in Eq.~(\ref{eq:bc_prop_post}) cancels out and only the unnormalized density up to that constant $p^*$ is required.
...
The acceptance probability is formulated to satisfy the \emph{detailed balance condition} (Eq.~(\ref{eq:ch5_markov_chain_detailed_balance})) for any valid proposal probability distribution and it guarantees the stationarity of the process \cite{Chib1995}.
