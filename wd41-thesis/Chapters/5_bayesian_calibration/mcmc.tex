...
Assuming a well-behaved function of interest under the integral, some regions of the state space will contribute more to the integration than the others.
...
In a high-dimensional case, the region of space around the mode contributes less and less mass in comparison to other part of the state space and thus becoming less typical. This is another example of the curse of dimensionality previously mentioned.}.
...
Specifically, a \emph{continuous-state} Markov chain with state space $\mathbf{X} \subseteq \mathbb{R}^D$ is defined as a sequence of random variables $\{\bm{\mathcal{X}}^{(i)}; i \geq 0\}$ where the indices represents successive time, step, or iteration,
