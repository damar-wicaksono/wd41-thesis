...
The tool \texttt{trace-simexp} has reached a stable version, is well documented, and has been applied for several follow-up studies within and outside the scope of the present doctoral research.
...
It does require an initial effort of getting acquainted with the terminologies of each framework, some are easier than others, but in the long run for a generic complex model these might be the preferred solution.
...
In the case studied here, more than half of the initial selection were found to be non-influential to the relevant outputs of the reflood simulation.

% FDA
In accordance with the aim of increasing the inputs/outputs relationship in a simulation understanding, a novel set of \glspl[hyper=false]{qoi} was derived using \gls[hyper=false]{fda} techniques to characterize the overall time-dependent output variation.
...
The resulting \glspl[hyper=false]{qoi} was then coupled with the \gls[hyper=false]{gsa} methods (Sobol' main- and total-effect indices) to investigate, quantitatively, the effect of the input parameters on the overall time-dependent outputs.
When considering FDA-based QoIs, which better represents the whole transient of an output, it was found that the important parameters and the nature of their interactions were changing during the transient.
...
However, landmarks might not always visible and miss-specification might affect the downstream analysis.
...
In such cases, an alternative approach to compute the indices are required.
...
The thesis has also demonstrated the applicability of a \gls[hyper=false]{pca} to reduce the high dimension of the output.
...
Finally, though many practical aspects were involved in the construction of the metamodel, the work in the thesis concluded that the size of the training sample (i.e., the actual code runs) was the most important factor;
...
% Main Achievements
Bayesian calibration to quantify the uncertainty of physical model parameters in \gls[hyper=false]{th} system codes has been successfully demonstrated for the selected \gls[hyper=false]{trace} reflood model parameters on the basis of the \gls[hyper=false]{trace} model of \gls[hyper=false]{feba}.
...
The uncertainty propagation of each resulting posterior samples was conducted on all \gls[hyper=false]{feba} tests and the results were compared in terms of \emph{informativeness} and \emph{calibration score}.

The value of incorporating model bias term in the calibration process has also been demonstrated, lest the calibration results exhibited stronger symptom of overfitting.
That is, although the prediction uncertainty band was narrower, more experimental data points fell outside the band.
...
Indeed, though better in terms of calibration score with respect to the scheme without model bias term, the posterior with bias term had consistently lower calibration scores across all \gls[hyper=false]{feba} tests compared to that of the the prior.
By removing a strongly correlated parameter from the calibration -- and keeping it at its prior uncertainty -- the resulting posterior prediction uncertainty was found to have much improved informativeness with similar level of calibration scores across all \gls[hyper=false]{feba} tests compared to that of the prior.
Therefore, it can be argued the calibration resulted in a posterior range and a posterior correlation structure which were, by construction, specific to the calibration data. 
However, if the data was deemed not sufficiently large or comprehensive enough (here it was based on one \gls[hyper=false]{feba} test run) then care should be taken to avoid overfitting. 
In this particular case, the calibration by excluding a strongly correlated parameter was proved to be a compromise and pragmatic solution.

Another type of parameter non-identifiability was also encountered during the calibration process.
The non-identifiability, due to parameter insensitivity with respect to a type of experimental data, was solved by employing calibration scheme that incorporated multiple types of experimental data simultaneously.
...
The applicability of the resulting posterior uncertainty is only applicable insofar the assumed bias from the calibration data is valid for the test data.
In the case of \gls[hyper=false]{feba}, different experimental conditions leads to different bias structure of the model which was apparent in the case of test Nos. $223$ and $218$.
Indeed the propagation of the posterior uncertainties performed poorly in such situation.
 
A more comprehensive calibration procedure therefore should take into account the difference in the bias structure from different experimental conditions.
One possibility is to concatenate all the experimental data of \gls[hyper=false]{feba} into a single calibration process resulting in a posterior uncertainties of model parameters that takes into account all available experimental boundary conditions of \gls[hyper=false]{feba}. 
However, to understand better the difference between the conditions, an alternative approach is the hierarchical modeling \cite{Unal2011} which allows the model parameters to take different posterior uncertainties depending on the experimental conditions, while at the same time allows the sharing of information from the data across different experimental conditions.
This approach might give a better insight on the model validity and reveal its discrepancy for a particular experimental condition in more precise manner.
...
Their calibration against experimental data are aimed at increasing the confidence in their application for the actual plant analysis (or to a lesser degree integral effect test facility).
...
Once more, in this thesis, the issue was sidestep by directly using directly the posterior samples for the uncertainty propagation thus implicitly captures the correlation structure.
It remains an open question on how to summarize these correlation structure for the purpose of uncertainty propagation.

% 
Regarding computational aspects of the Bayesian calibration, \gls[hyper=false]{mcmc} sampler is the backbone for modern analysis.
...
For robustness, it is necessary to extend the verification studies of using different \gls[hyper=false]{mcmc} samplers.