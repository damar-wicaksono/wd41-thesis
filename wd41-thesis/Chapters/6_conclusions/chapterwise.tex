...
The particular test series selected corresponded to the case without flow blockage, under three different values of system backpressure boundary condition (between $2.1, 4.1,$ and $6.2\,[bar]$) and two different reflood rates ($3.8$ and $5.8\,[cm\cdot s^{-1}]$).
...
% Chapter 3
Chapter~\ref{ch:gsa} introduced selected \glsfirst[hyper=false]{gsa} methods which were then applied to the \gls[hyper=false]{trace} model of \gls[hyper=false]{feba}.
...
The results regarding parameter sensitivity with respect to different outputs have provided a better understanding on the inputs/outputs relationship in the \gls[hyper=false]{trace} model of \gls[hyper=false]{feba}.
...
All in all, the calibration schemes corresponded to different cases of trade-off between informativeness (the width of the prediction uncertainty band) and calibration score (whether the experimental data covered by the uncertainty band).
This trade-off was apparent particularly for the schemes with and without model bias and the scheme with bias but excluding a strongly correlated parameter.
The scheme without bias resulted in most reduction of the prior uncertainty for most of the important parameters, in which the nominal \gls[hyper=false]{trace} parameter values sometimes laid outside the posterior uncertainty interval.  
...
The scheme with bias resulted in a more modest reduction of the prior uncertainty of the parameters, keeping the nominal \gls[hyper=false]{trace} parameter values within the uncertainty interval, but exhibited strong correlation for some of the parameters.
...
It could be argued that the relatively worse calibration scores for the scheme with and without bias, in comparison with that of the prior, was due to the too narrow posterior uncertainties for the former and due to the correlation in the posterior uncertainties for the latter.
