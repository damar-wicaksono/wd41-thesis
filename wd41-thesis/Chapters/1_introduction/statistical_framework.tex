...
An increasingly more complex computer simulator, however, often translates to a long running simulation (i.e., computationally expensive) or requiring vast amount of evaluations due to complexity of relationship between high-dimensional inputs and high-dimensional outputs, or both.
They, in turn, hinder the analyses and the effort to achieve the aforementioned objectives of computer experiment.

% Why Metamodel
As a result, having a fast approximating model of a complex simulator is important in conducting a computer experiment and its value was acknowledged by Sacks et al. in their seminal paper \cite{Sacks1989} and formalized further in several textbooks \cite{Santner2003,Fang2006}.
...
This approximating model in the literature is referred to as \emph{metamodel}, \emph{surrogate model}, \emph{response surface model}, or \emph{emulator}.

% Broad classification
In the literature, any of the terms above are used interchangeably though subtle difference exists.
Thus, it is worthwhile to consider a broad classification of metamodels and metamodeling approach according to the literature.
\marginpar{Classification}
Metamodels according to its derivation (i.e., metamodeling) can be broadly classified in two categories: the data-driven \emph{response surface surrogates} and the mechanistic \emph{reduced-order models} \cite{Razavi2012}.
Reduced order modeling is perhaps more familiar in the scientific community where a complex physical model is being simplified by putting more stringent assumption or reducing the numerical resolution while trying to preserve the most important physical processes present in the more complex model.
\marginpar{Reduced-order model}
The point kinetics model is an example of a reduced-order model, substituting the more complex $3$-dimensional nodal code;
...
% Gaussian Process, Why and recent popularity due to machine learning
