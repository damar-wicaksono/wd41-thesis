...
The set of strategies above, for sensitivity analysis, model approximation, and calibration presented above constitutes a consolidated statistical framework that will be used in this thesis for quantifying the uncertainty in model parameters of a \gls[hyper=false]{th} system code.
In the following we present a broad, and by no means exhaustive, overview on the research landscape from the literature on each of the elements of the proposed strategy.
For each of these elements, the overview will first reiterate its main motivation followed by the presentation of its classification and the steps involved in a typical application before briefly summarizing its applications in the nuclear engineering \gls[hyper=false]{th} analysis, both in the past and more recent times.
As will be outlined in Section~\ref{sec:intro_thesis_structure}, three chapters of the thesis will later be dedicated for each of the proposed strategies detailing the selected methods further as well as presenting their application on a \gls[hyper=false]{trace} model.
...
% Introductory Paragraph
An essential part of model development and assessment is to properly describe and understand the impact of model parameter variations on the model prediction.
\Gls[hyper=false]{sa} is an important methodological step in that context \cite{Trucano2006}.
\gls[hyper=false]{sa} is the process of investigating the role of input parameters in determining the model output \cite{Iooss2015} variation. 
...
When applicable, reduced-order models can be useful as first approximation and didactic tools to build intuition. 
...
% Introductory paragraph
The objective of model calibration is to increase the agreement between simulation prediction and its corresponding measurement data by adjusting some of the simulator inputs \cite{Campbell2006a,Trucano2006}.
...
It is adapted here to deal with the particular problem posed in \gls[hyper=false]{premium} benchmark that itself represents typical problem in the nuclear engineering \gls[hyper=false]{th} analysis, as detailed in Chapter~\ref{ch:bayesian_calibration}.
...
This probabilistic model is then conditioned on the observed data that results in the likelihood function.
...
% Regarding prior
Regarding the choice for the prior distribution, there is a tendency in the modern literature \cite{Gelman2013,McElreath2015} to move away from the notion of \emph{Bayesianism} where emphasis is put heavily on the prior distribution of the parameters as \emph{subjective opinion} and the results will always be true (i.e., coherent) relative to that prior.
\marginpar{On prior distribution}
On the contrary, the use of prior should be seen in more pragmatic light, that is, as a starting assumption that can be changed if not appropriate.
In the current practice, the choice for prior can be provisional, especially for parameters associated with the probabilistic model (not the simulator itself) \cite{Gelman2002,Gelman2006}.
...
Numerically integrating the posterior derive \glspl[hyper=false]{qoi} might not be efficient.
Traditionally, maximum a posteriori estimates (or maximum likelihood, if the prior is non-informative) is often used to describe each of the posterior model parameter value in a single number.
\marginpar{Semi analytical approach}
It simply requires the maximization of the posterior density function (i.e., its mode).
An extension to this, giving description of the shape of the posterior, is done by the so-called \emph{Laplace's approximation} or the \emph{normal approximation} \cite{Mackay2005,Sivia2012,Gelman2013,McElreath2015}.
In the approximation, the distribution of the posterior is approximated as a normal distribution. 
...
The approximation works well if the posterior distribution is approximately normal (i.e., unimodal, bell-shaped, linearly correlated).
...
In the latter, generation of a \emph{Markov chain} by \gls[hyper=false]{mc} simulation paved a way to a generic simulation method applicable to generate samples from any kind of probability distribution; thus the origin of \gls[hyper=false]{mcmc} simulation.
Later on, its usage for data analysis in general and Bayesian data analysis in particular were revived in the $1970$s and the early $1980$s by the generalization of the simulation algorithm by Hastings \cite{Hastings1970} (i.e., the \gls[hyper=false]{mh} algorithm) and by the invention of a computationally efficient sampler and its application for image reconstruction problem by Geman and Geman \cite{Geman1984} (i.e., the Gibbs sampler).
...
Hamiltonian \gls[hyper=false]{mc} sampler simulates the movement of a particle in the parameter space as described by the posterior distribution according to Hamiltonian dynamic.
...
% Development in nuclear engineering application
Unlike the forward uncertainty propagation and statistical sensitivity analysis, Bayesian model calibration is relatively recent to be applied in nuclear engineering application \cite{Unal2011}.
\marginpar{Developments in nuclear engineering application}
Notable early example was the previously mentioned CIRC\'E \cite{Crecy2001} for the calibration of model parameters in a \gls[hyper=false]{th} system code.
Recent demonstration on the applicability of the method can be found in Ref.~\cite{Freixa2016}.
More recent examples are more in-line to the KOH approach with additional extension:
...
with \cite{Li2017} or without \cite{Heo2018} the use of metamodels.
