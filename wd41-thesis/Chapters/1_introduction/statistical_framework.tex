...
Under the Bayesian interpretation, the metamodel assumes a prior distribution over a family of functions to describe an unknown (possibly complex) function that underlies the simulator.
...
This measure of uncertainty makes a \gls[hyper=false]{gp} metamodel a suitable choice to be incorporated into a statistical model calibration framework where multiple sources of uncertainty are taken into account.
%Thus, this research adopts \gls[hyper=false]{gp} for constructing a metamodel of a \gls[hyper=false]{th} system code model and Chapter~\ref{ch:gp_metamodel} is dedicated for the description.
