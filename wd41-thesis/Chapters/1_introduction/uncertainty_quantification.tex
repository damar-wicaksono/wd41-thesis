...
\begin{enumerate}
	...
	\item Uncertainty associated with the controllable.
  ...
  \item Uncertainty associated with the physical model parameters.
	...
	They can either be estimated using an experimental data or expert judgment.
	
  \item Uncertainty associated with the physical models.
...
The conceptual distinction related to these parameters will be revisited in detail in Chapter~\ref{ch:bayesian_calibration}.
...
Many physical model parameters cannot be measured directly.
...
this can also apply to phenomenological models where some parameters are allowed to be tuned according to match the experimental data.
...
In this setting, given a set of experimental data $\{\mathbf{D}\}$ taken with known controllable inputs $\mathbf{x}_c$, the task is then to infer the value of the parameters in the physical model used to predict the same quantity as the experimental data.
It is important here to acknowledge various sources of uncertainty previously mentioned:
...
% Bayesian framework
In a probabilistic setting, a natural way to make an inference of unobserved parameters based on observed data is through the Bayes' theorem,
\begin{equation*}
...
However, instead of a single value (or values in case of multiple parameters), the resulting posterior is a \gls[hyper=false]{pdf}, conditioned on the observed data.
...
The scope of the project was limited to the physical models in system code to simulate bottom reflood phenomena.
...
Thus the main goal of the project followed the approach of statistical uncertainty analysis explained above.
