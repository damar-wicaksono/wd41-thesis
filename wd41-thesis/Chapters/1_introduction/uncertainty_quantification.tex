...
Some of those specifications (such as the boundary conditions, etc.) are parametrized and constitutes \emph{controllable inputs} denoted by $\bm{x}_c$\footnote{later on, \emph{controllable} inputs corresponds to equivalent parameters in a physical experiment which can be controlled by the experimentalist.}.
