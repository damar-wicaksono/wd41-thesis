...
The resulting random code outputs can be summarized by different integral quantities.
For instance, the mean
\begin{equation*}
	\mathbb{E}[f] = \int\limits_{\mathbf{X}_c,\mathbf{X}_m} f(\bm{\mathcal{X}}_c, \bm{\mathcal{X}}_m;\bm{r}, t) \, p(\bm{x}_c, \bm{x}_m) \, d\bm{x}_c \, d\bm{x}_m
\end{equation*}
where $p(\bm{x}_c, \bm{x}_m)$ denotes the joint \gls[hyper=false]{pdf} for the input parameters.

Using \gls[hyper=false]{mc} technique, samples are generated from their respective distributions and they are used to run the code multiple times.
...
Finally, there is a natural reluctance among the participants to embrace more advanced methods requiring less assumptions (e.g., normal prior of the parameters, linearity between outputs and parameters, etc.) but with more computational cost (i.e., more code runs).
\marginpar{The use of metamodel}
computational cost (i.e., more code runs) that are available in the recent literature.
