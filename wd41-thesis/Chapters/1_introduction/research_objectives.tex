...
\begin{enumerate}
  ...
  An uncertainty analysis often starts with large list of input parameters that might and might not be relevant (i.e., influential) to the simulation at hand.
  A screening method can be used to reduce the number of the initial input parameters to a limited set of only the influential ones.
  Afterward, a variance decomposition method is employed to quantitatively analyze the contribution of the influential parameters uncertainty on the prediction uncertainty.
	As a test facility might have multiple types of data (cladding temperature, pressure drop, etc.), it might be worthwhile to consider each one of them; 
  although the information content might not be the same for the different types.
  ...
  Section~\ref{sub:intro_sa} provides an overview on the wide range of \gls[hyper=false]{sa} methods from the applied literature,
  while Chapter~\ref{ch:gsa} presents the details on the selected \gls[hyper=false]{sa} methods and their application to a \gls[hyper=false]{trace} model.

% Aim 2 (Statistical Metamodeling)
	\item \emph{to approximate} the inputs/outputs relationship of a complex computer simulation for a faster evaluation.
  ...
  In the subsequent section (Section~\ref{sub:intro_statistical_metamodeling}), a broad overview on metamodeling in the literature is first introduced;
  later on, in Chapter~\ref{ch:gp_metamodel}, the details and the application of \glsfirst[hyper=false]{gp} metamodel to a \gls[hyper=false]{trace} model are presented.
...
% Aim 3 (Bayesian Calibration)
  ...
  The practice of Bayesian calibration of computer model from the literature is provided in Section~\ref{sub:intro_bayesian_calibration}.
  Chapter~\ref{ch:bayesian_calibration} is then dedicated to the details of formulating a Bayesian calibration problem for model parameters, ways to solving it, and an application of it to a \gls[hyper=false]{trace} model.
