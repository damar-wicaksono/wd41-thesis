...
They provide a \emph{physics-based} evaluation of a relevant phenomenon taking place in the plant during postulated transients to demonstrate that safety requirements are met \cite{IAEA2009}.
...
the thermo-mechanics of the fuel and the structure;
...
Each discipline is, in turn, characterized by a distinct set of governing physical equations and often analyzed by a distinct code.
...
Whether the physical quantities respect such limits during postulated scenarios is analyzed using simulations in a conservative or best-estimate approach \cite{IAEA2009}.
...
Assuming less interfacial shear between liquid and gas phases of the coolant (water) reduces mist flow and is a conservative assumption because less heat is transferred to the coolant flow in the upper region of the core.
which penalizes the fuel temperature prediction.
...
As opposed to the conservative approach, \emph{best-estimate} approach calls for (more) physically sound thermal-hydraulics models with more realistic assumptions, which are backed by experimental data obtained from numerous experimental programs conducted in Separate and Integral Effect Test Facilities.
...
The system includes the reactor pressure vessel with hundreds of fuel assemblies; kilometers of interconnecting pipes; scores of valves, pumps, and tanks; as well as numerous special components like steam generator and condenser.
...
As can be seen, a system code constitutes of several building blocks that can be used to model and simulate wide ranges of systems and conditions.
...
System codes are complemented with models for special components that perform specific functions (e.g., solid heated structure, pumps, and separators) or actions during transients (e.g., valves, instrumentation, and control systems);
and models for special processes and phenomena that are relevant to the \glspl[hyper=false]{lwr} but too complex to be modeled implicitly in the (simplified) conservation equations (e.g., critical flow).
...
The model can capture phenomena where thermal and mechanical non-equilibrium conditions exist between the two phases, giving more realistic pictures in a wide range of transients.
