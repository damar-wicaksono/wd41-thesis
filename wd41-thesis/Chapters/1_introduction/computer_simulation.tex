...
\marginpar{Simulator}
As a result, the term \emph{simulator} includes not only the code itself, but also the particular system of interest being modeled using the code \cite{OHagan2006}.
...
a \emph{physics-based} evaluation of relevant phenomena taking place in the plant during broad range of postulated transients to demonstrate that safety requirements of operation are met \cite{IAEA2009}.
The demonstration is carried out with respect to acceptance criteria, a set of limits and conditions to ensure the integrity of the safety barrier.
The criteria are set by regulatory bodies for all operational states of the plant, normal and off-normal.

% Deterministic safety analysis
The physics-based evaluation, on the other hand, is achieved by the use of scientific codes to predict the response of the plant to a postulated transient scenario.
...
Each of these disciplines (or domain) is, in turn, characterized by a distinct set of governing physics of the process involved and thus often analyzed by a distinct code.

% Entry to Conservative vs Best Estimate
The connection between the physics-based evaluation and the de\-monstration of safety are established, among other things, by setting the acceptance criteria in terms of limiting physical quantities relevant in  phenomena involved.
The upper tolerance limit of $1'204\,[^oC]$ for the \glsfirst[hyper=false]{pct} is one such a criteria for \glspl[hyper=false]{lwr} \cite{USNRC2017}.
...
Conservatism called for the most pessimistic and penalizing modeling assumptions (including the initial and boundary conditions) to ensure conservative results, which is far below their regulatory limits.
...
Fully empirical laws are developed based only on the available representative experimental data by directly correlating observed transfer terms of interest with observed flow variables.
...
As such, these parameters become a measure of the inadequacy of the postulated model in explaining the data due to any missing physical process, unaccounted for.
