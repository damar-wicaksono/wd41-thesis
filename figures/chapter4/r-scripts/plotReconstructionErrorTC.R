#
# title     : plotReconstructionErrorTC.R
# purpose   : R script to create a plot of reconstruction error for the 
#           : clad temperature output (at 8 different locations) 
...
# Global variables ------------------------------------------------------------
# Output filename
otpfullname <- "figures/plotReconstructionErrorTC.pdf"
...
n_ts <- 9999        # Number of time steps
n_outp <- 8         # Number of outputs (4 for four different segments of DP)
idx_outp <- 1:8     # Index of the output (1 to 4 for dp_smoothed)
...
for (i in 1:n_valid) yy_valid[i,] <- trc_valid$replicates[2:length(trc_valid$time), i, idx_outp]
rm(trc_valid)
