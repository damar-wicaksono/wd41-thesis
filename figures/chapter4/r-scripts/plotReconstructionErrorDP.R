#
# title     : plotReconstructionErrorDP.R
# purpose   : R script to create a plot of reconstruction error for the 
#           : pressure drop output (at 4 different segments) 
#           : using increasing number of principal components in 
#           : the reconstruction
...
# Global variables ------------------------------------------------------------
# Output filename
otpfullname <- "figures/plotReconstructionErrorDP.pdf"

# Input data filename
trc_valid_path <- "../../../../wd41-thesis.analysis.new/postpro-gp-training/result-compiled"
trc_valid_filename <- "febaTrans216Ext-febaVars12Influential-lhs_5000_12_valid-dp_smoothed.Rds"

trc_train_pca_path <- "../../../../wd41-thesis.analysis.new/trace-gp/result-pc"
trc_train_pca_filename <- c(
    "febaTrans216Ext-febaVars12Influential-sobol_1920_12-dp_smoothed-pca.Rds")

n_train <- 1920     # Number of training samples
n_valid <- 5000     # Number of validation samples
n_ts <- 10000       # Number of time steps
n_outp <- 4         # Number of outputs (4 for four different segments of DP)
idx_outp <- 1:4     # Index of the output (1 to 4 for dp_smoothed)
