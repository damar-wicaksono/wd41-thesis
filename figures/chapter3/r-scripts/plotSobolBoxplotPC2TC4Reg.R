#
# title     : plotSobolBoxplotPC2TC4Reg.R
# purpose   : Script to create a boxplot of Sobol Indices for each parameter
#           : with PC2 of registered mid-assembly temperature output as QoI
...
# Global variables ------------------------------------------------------------
# Input filename
otpfullname <- "./figures/plotSobolBoxplotTC4RegPC2.pdf"

# Output filenames
data_path <- "../../../analysis/sobol/results"
bootstrap_files <- c(
    paste0(data_path, "/", "bootstrap-saltelli-tc_4_regfd-pc2-2000.csv"),
    paste0(data_path, "/", "bootstrap-sti-tc_4_regfd-pc2-2000.csv"))
indices_file <- paste0(data_path, "/", "estimate-tc_4_regfd-pc2-2000.csv")

idx_estimator <- 1  # Use the Saltelli et al. estimator
...
p <- p + scale_y_continuous(breaks = seq(-0.1, 0.8, 0.1),
                            limits = c(-0.1, 0.8))

# Calculate the sum of first order indices
sum(indices_df[, idx_estimator]) # 0.27

# Make a title informing the sum of the first order indices
first_line <- "Sum of the main effect indices = 0.27"
