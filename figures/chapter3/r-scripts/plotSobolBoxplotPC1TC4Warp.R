#
# title     : plotSobolBoxplotPC1TC4Warp.R
# purpose   : Script to create a boxplot of Sobol Indices for each parameter
#           : with PC1 of TC4 warping function as the QoI
...
# Global variables ------------------------------------------------------------
# Output filename
otpfullname <- "./figures/plotSobolBoxplotPC1TC4Warp.pdf"

# Input filenames
data_path <- "../../../analysis/sobol/results"
bootstrap_files <- c(
    paste0(data_path, "/", "bootstrap-saltelli-tc_4_warpfd-pc1-2000.csv"),
    paste0(data_path, "/", "bootstrap-sti-tc_4_warpfd-pc1-2000.csv"))
indices_file <- paste0(data_path, "/", "estimate-tc_4_warpfd-pc1-2000.csv")
...
sum(indices_df[, idx_estimator]) # 0.93

# Make a title informing the sum of the first order indices
first_line <- "Sum of the main effect indices = 0.94"
