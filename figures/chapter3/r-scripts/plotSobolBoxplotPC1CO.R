...
# Input filenames
bootstrap_files <- c(
    "../../../analysis/sobol/results/bootstrap-saltelli-co-pc1-2000.csv",
    "../../../analysis/sobol/results/bootstrap-sti-co-pc1-2000.csv")
indices_file <- c(
    "../../../analysis/sobol/results/estimate-co-pc1-2000.csv")
...
p <- p + scale_y_continuous(breaks = seq(-0.1, 1.0, 0.1),
                            limits = c(-0.1, 1.0))

# Calculate the sum of first order indices
sum(indices_df[, idx_estimator]) # 0.98

# Make a title informing the sum of the first order indices
first_line <- "Sum of the main effect indices = 0.98"
