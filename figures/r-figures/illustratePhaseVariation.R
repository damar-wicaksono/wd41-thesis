...
plot(y1_smooth)
lines(t, eval.fd(t, mean.fd(y1_smooth$fd)), lwd = 4)
...
test_reg_0 <- landmarkreg(y1_smooth$fd, landmark, landmark_ref, WfdPar=WfdPar, monwrd = TRUE)

plot(test_reg_0$warpfd, col = rgb(0, 0, 0, 0.25), lty = 1)

# 
png("illustrateRegisteredCurves.png", width=22, height=11, units="cm", res=600)
par(mfrow=c(1, 2), mar=c(4,4,1,1) + 0.1, las = 1, oma = c(0,0,0,0) + 0.1)
plot(0, 0, xlim = c(-0.025, 1.025), ylim = c(0,3.1), type = "n",
     yaxt = "n",
     xaxt = "n",
     xlab = "",
     ylab = "")
...
lines(t, eval.fd(t, mean.fd(y1_smooth$fd)), lwd = 3)
lines(t, theta(t), lwd = 2, lty = 2, col = "red")
# 
#lines(t, eval.fd(t, mean.fd(y1_smooth$fd)), lwd = 3, col = "red")

# reg compare
plot(y1_smooth$fd[15], lty = 2)
lines(test_reg_0$regfd[15])
lines(t, theta(t), lwd = 2, lty = 2)
#
